...
unsigned char *readsectorB(void)
{
    int busy;
...
void loaddirinfo(int startSector, int numSectors, int maxEntries)
{
    CD_ctx_t      *ctx = &Cdinfo;
...
int CD_Init(int maxOpen, int numEntries, void *buffer, void (*callback)(int))
{
    CdlLOC         toc[2];
    unsigned char *root;
    int            rootExtent;
...
int CD_Open(char *name, int flags, int *outp)
{
    char   upper[16];
    char  *p    = upper;
    void **h;
...
int CD_Read(int dev, int dest, int offset, int len)
{
    ...
    if (CD_cachedSector == CD_curSector && (CD_info & 0x10) && g_currentthread[0] == 2) {
        { char *cache = (char *)CD_sectorCache;
          int off;
          /* W85-S7 (2026-09-02): the w47-a5 zero-insn use fence `__asm__("" : : "r"(cache))`
           * that used to sit here is GONE and the fn still PASSes -- the sched1 tie it was
           * pinning is already decided the oracle's way by the SPLIT `off = rs->curOff;
           * off += (int)cache;` statement pair (the cache base is materialized before the
           * curOff load because it is the addend, not the loaded value).  Re-measured on the
           * source-only gate: CD_Read PASS, whole TU 14/14 PASS with the fence deleted.
           * Do NOT re-fuse the two statements into `off = rs->curOff + (int)cache;`. */
          off = rs->curOff;
          off += (int)cache;
...
int CD_Getinfo(int handle, int namebuf, int *sizeout)
{
