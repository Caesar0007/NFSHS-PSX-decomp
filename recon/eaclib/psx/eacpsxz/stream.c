 ...
 * load-delay nop.
 * W85-S7 DEVICE PURITY (2026-09-02): the record pointer used to be declared `volatile
 * StreamFilter *f` with a `f++` pointer-walk.  That volatile was a CODEGEN CRUTCH for a
 * GIV-ANCHOR problem, not a semantic property (the filter table is plain read-only config).
 * Without it the pointer-walk lets loop.c strength-reduce the three field addresses onto a
 * SECOND walking giv -- `addiu a2,a0,8` + `lw v1,-4(a2)` + `addiu a2,a2,12`, 21 insns vs the
 * oracle's 20 -- where retail keeps ONE base with displacements 0/4/8 and a single
 * `addiu a0,a0,12` (catalog SS-B "loop.c never ran").  FIX = the INDEX FORM `f[i].field`
