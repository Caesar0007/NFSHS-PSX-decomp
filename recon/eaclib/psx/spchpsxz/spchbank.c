...
/* the bank allocator state (gVoxBanks, gNumBanks, gClearCycle, gCDErrors) is owned by spchdata.obj (spchdata.c) */
...
VoxBank **iSPCH_BankMemAlloc(int numBanks)
{
    if(!gVoxBanks) {
        gNumBanks = numBanks;
 ...
 *   or -1 if not found. */
int iSPCH_FindBank(unsigned short key)
{
    int i;
...
bool iSPCH_TestSubBankBounds(int bankIdx, int subIdx)
{
    bool result = false;
...
void iSPCH_SetCycleBits(VoxBank *bank)
{
    char *bits;
    int   nGroups;
