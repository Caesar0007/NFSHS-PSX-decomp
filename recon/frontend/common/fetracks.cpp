...
/* ---- tTrackManager::LoadDescription  [FETRACKS.CPP:94-139] native locals verified; SLD open ---- */
...
/* MATCH: the SYM-authenticated source has only `input`, `data`, `filename`,
   ...
   `data` is the file payload at input+4, while the allocation result goes
   directly to fTracks. The indexed record loop is strength-
...

{
  ...
  data = input + 4;
  this->fTracks = (tTrackInformation *)reservememadr("Track List",this->fNumTracks * 0x30,0);
  blockmove(data,this->fTracks,this->fNumTracks * 0x30);
  i = 0;
...
{
  tTrackInformation *trackInfo = &this->fTrackManager->fTracks[(u_char)this->fValue[(u_char)*this->fIndex]];
  return (signed char)trackInfo->fTrackID + 0xd5;
}
...

{
  do {
    ...
    if (this->fValue[(u_char)*this->fIndex] >= this->fTrackManager->fNumTracks) {
...

{
  do {
    ...
    if (this->fValue[(u_char)*this->fIndex] == 0) {
      this->fValue[(u_char)*this->fIndex] = this->fTrackManager->fNumTracks - 1;
    } else { this->fValue[(u_char)*this->fIndex] = this->fValue[(u_char)*this->fIndex] - 1; }
  } while (!this->ValidTrack(this->fValue[(u_char)*this->fIndex]));
}
...
{
  tTrackInformation *trackInfo = this->fTrackManager->fTracks + (u_char)track;
  bool result = this->fTrackManager->fAvailableTracks[(signed char)trackInfo->fTrackID];
