...
{
  /* decl order = SYM Def-record order (tempShp2, fileName); `artfilename` is a
     NESTED-block local (SYM `90 Block start line = 9`) -- w86-S5 */
  shapetbl *tempShp2;
  char fileName [48];
...

{
  ...
  /* decl order = SYM Def-record order (prim,width,height,u,v,uw,vh,bpp);
     the non-SYM carrier follows it -- w86-S5 */
  POLY_GT4 *prim;
  short    width;
  short    height;
  char     u;
...

{
  ...
  /* decl order = SYM Def-record order (scalex,scaley,tShp,color) -- w86-S5 */
  int scalex = 0x20000;
  int scaley = 0x10000;
  tTexture_ShapeInfo *tShp;
  int color [4];
...
{
  /* decl order = SYM Def-record order (fullName, permFile, i) -- w86-S5 */
  char fullName [48];
  char *permFile;
  int i;
