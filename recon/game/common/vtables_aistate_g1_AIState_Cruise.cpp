/* game/common/vtables_aistate.cpp -- #75 data-materialization: AIHigh-family vtables (12).
 * Real nfs4-f.exe bytes; __vtbl_ptr_type (size=_vf[4]); pfn VAs symbolicated. @0xVA breadcrumbs.
 * NOT original layout (the obj owned each in its rodata); self-contained + faithful + links. */
#include "../../nfs4_types.h"
extern "C" int __pure_virtual(...);   /* @0x800e4354 (eaclib cfront runtime) */

/* w66-a2: retail's dtor slot holds a REAL destructor symbol (read per slot out
 * of asm/data/*.s at the slot VA+4 -- for a class with no declared dtor that is
 * an ANCESTOR's `___<len><Base>`, w65-a3's DTOR-DEPTH LAW).  The slots below name
 * those symbols directly; the fabricated per-class wrappers
 * `static int wrap(X *p){ p->~X(); return 0; }` (an artifact of C++ forbidding
 * `&Class::~Class`) are gone. */
/* aistate.obj carries its OWN out-of-line copies of the header-inline
 * AIState_Base::TestForRelease / AIState_NonActive::Execute/~ (retail
 * 0x80072830/0x80072750/0x80072758; aihigh.obj and aih_btccop.obj have
 * their own).  The aistate vtables point at aistate's copies; only the
 * AIState_None table (which lives in aihigh.obj's .rdata) uses aihigh's. */
extern "C" int TestForRelease__12AIState_Base_80072830(...);
extern "C" int Execute__17AIState_NonActive_80072750(...);
extern "C" void ___17AIState_NonActive_80072758(void *thisp);
extern "C" void ___12AIState_Base_80072838(void *thisp);   /* ~AIState_Base */
/* SS3.23 real-deleting-dtor free fns (___<N><Class>(Class*,int __in_chrg), aistate.cpp) --
   these 10 are no longer C++ member dtors (a real member dtor for this non-polymorphic
   single-inheritance shape always compiles to gcc's simple base-forward, not the oracle's
   per-class deleting dtor), so the vtable dtor slot points at the free fn directly; the
   __in_chrg=2 convention here (matching the "sub-object being destroyed, don't delete" call
   from a further-derived class's own dtor) mirrors the raw dtor-slot ABI: index 2 in the
   vtable always holds a fn taking (self,__in_chrg) and deciding whether to __builtin_delete. */
extern "C" void ___13AIState_Chase(AIState_Chase*,int);
extern "C" void ___14AIState_Cruise(AIState_Cruise*,int);
extern "C" void ___14AIState_Donuts(AIState_Donuts*,int);
extern "C" void ___17AIState_GotoSlice(AIState_GotoSlice*,int);
extern "C" void ___12AIState_Idle(AIState_Idle*,int);
extern "C" void ___17AIState_NonActive(AIState_NonActive*,int);
extern "C" void ___12AIState_None(AIState_None*,int);
extern "C" void ___14AIState_Normal(AIState_Normal*,int);
extern "C" void ___15AIState_Offroad(AIState_Offroad*,int);
extern "C" void ___17AIState_Purgatory(AIState_Purgatory*,int);
extern "C" void ___21AIState_RovingTraffic(AIState_RovingTraffic*,int);

/* Blocks sorted by retail address (= g++ finish_file emission order, newest
 * class first) and SPLIT into contiguous retail runs, one TU per run, so each
 * run is placed/validated at its own retail base.  Do not merge or re-sort. */
__vtbl_ptr_type AIState_Cruise_vtable[4] = {   /* @0x80055500 */
  {0, 0, (int (*)(...))0},                           /* @0x80055500  null */
  {0, 0, (int (*)(...))&AIState_Cruise::Execute},    /* @0x80055508  Execute__14AIState_Cruise */
  {0, 0, (int (*)(...))&___14AIState_Cruise},     /* @0x80055510  ~AIState_Cruise */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x80055518  TestForRelease__12AIState_Base */
};
__vtbl_ptr_type AIState_GotoSlice_vtable[4] = {   /* @0x80055520 */
  {0, 0, (int (*)(...))0},                           /* @0x80055520  null */
  {0, 0, (int (*)(...))&AIState_GotoSlice::Execute}, /* @0x80055528  Execute__17AIState_GotoSlice */
  {0, 0, (int (*)(...))&___17AIState_GotoSlice},  /* @0x80055530  ~AIState_GotoSlice */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x80055538  TestForRelease__12AIState_Base */
};
__vtbl_ptr_type AIState_Donuts_vtable[4] = {   /* @0x80055540 */
  {0, 0, (int (*)(...))0},                           /* @0x80055540  null */
  {0, 0, (int (*)(...))&AIState_Donuts::Execute},    /* @0x80055548  Execute__14AIState_Donuts */
  {0, 0, (int (*)(...))&___14AIState_Donuts},     /* @0x80055550  ~AIState_Donuts */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x80055558  TestForRelease__12AIState_Base */
};
__vtbl_ptr_type AIState_RovingTraffic_vtable[4] = {   /* @0x80055560 */
  {0, 0, (int (*)(...))0},                           /* @0x80055560  null */
  {0, 0, (int (*)(...))&AIState_RovingTraffic::Execute}, /* @0x80055568  Execute__21AIState_RovingTraffic */
  {0, 0, (int (*)(...))&___21AIState_RovingTraffic}, /* @0x80055570  ~AIState_RovingTraffic */
  {0, 0, (int (*)(...))&AIState_RovingTraffic::TestForRelease}, /* @0x80055578  TestForRelease__21AIState_RovingTraffic */
};
__vtbl_ptr_type AIState_Purgatory_vtable[4] = {   /* @0x80055580 */
  {0, 0, (int (*)(...))0},                           /* @0x80055580  null */
  {0, 0, (int (*)(...))&AIState_Purgatory::Execute}, /* @0x80055588  Execute__17AIState_Purgatory */
  {0, 0, (int (*)(...))&___17AIState_Purgatory},  /* @0x80055590  ~AIState_Purgatory */
  {0, 0, (int (*)(...))&AIState_Purgatory::TestForRelease}, /* @0x80055598  TestForRelease__17AIState_Purgatory */
};
__vtbl_ptr_type AIState_NonActive_vtable[4] = {   /* @0x800555a0 */
  {0, 0, (int (*)(...))0},                           /* @0x800555a0  null */
  {0, 0, (int (*)(...))&Execute__17AIState_NonActive_80072750}, /* @0x800555a8  Execute__17AIState_NonActive */
  {0, 0, (int (*)(...))&___17AIState_NonActive_80072758},  /* @0x800555b0  ~AIState_NonActive */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x800555b8  TestForRelease__12AIState_Base */
};
__vtbl_ptr_type AIState_Offroad_vtable[4] = {   /* @0x800555c0 */
  {0, 0, (int (*)(...))0},                           /* @0x800555c0  null */
  {0, 0, (int (*)(...))&AIState_Offroad::Execute},   /* @0x800555c8  Execute__15AIState_Offroad */
  {0, 0, (int (*)(...))&___15AIState_Offroad},    /* @0x800555d0  ~AIState_Offroad */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x800555d8  TestForRelease__12AIState_Base */
};
__vtbl_ptr_type AIState_Chase_vtable[4] = {   /* @0x800555e0 */
  {0, 0, (int (*)(...))0},                           /* @0x800555e0  null */
  {0, 0, (int (*)(...))&AIState_Chase::Execute},     /* @0x800555e8  Execute__13AIState_Chase */
  {0, 0, (int (*)(...))&___13AIState_Chase},      /* @0x800555f0  ~AIState_Chase */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x800555f8  TestForRelease__12AIState_Base */
};
__vtbl_ptr_type AIState_Idle_vtable[4] = {   /* @0x80055600 */
  {0, 0, (int (*)(...))0},                           /* @0x80055600  null */
  {0, 0, (int (*)(...))&AIState_Idle::Execute},      /* @0x80055608  Execute__12AIState_Idle */
  {0, 0, (int (*)(...))&___12AIState_Idle},       /* @0x80055610  ~AIState_Idle */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x80055618  TestForRelease__12AIState_Base */
};
__vtbl_ptr_type AIState_Normal_vtable[4] = {   /* @0x80055620 */
  {0, 0, (int (*)(...))0},                           /* @0x80055620  null */
  {0, 0, (int (*)(...))&AIState_Normal::Execute},    /* @0x80055628  Execute__14AIState_Normal */
  {0, 0, (int (*)(...))&___14AIState_Normal},     /* @0x80055630  ~AIState_Normal */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x80055638  TestForRelease__12AIState_Base */
};
__vtbl_ptr_type AIState_Base_vtable[4] = {   /* @0x80055640 */
  {0, 0, (int (*)(...))0},                           /* @0x80055640  null */
  {0, 0, (int (*)(...))&__pure_virtual},             /* @0x80055648  __pure_virtual */
  {0, 0, (int (*)(...))&___12AIState_Base_80072838},       /* @0x80055650  ~AIState_Base */
  {0, 0, (int (*)(...))&TestForRelease__12AIState_Base_80072830}, /* @0x80055658  TestForRelease__12AIState_Base */
};
