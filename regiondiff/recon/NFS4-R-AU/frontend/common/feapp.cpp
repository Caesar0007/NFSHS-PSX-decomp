...
/* REGIONAL (NFS4-R-AU): region-only frontend overlay helper @0x8004DCB4.
 * Mechanical func_<region_va> name (§8.3 no-auto-naming). */
...

{
  ...
  char saveFPlayer;
  short height;
  /* REGIONAL (NFS4-R-AU): char buffer[32] removed with the debug overlays. */
  DRAWENV *drenv;
  DR_AREA *daprim;
  ...
  /* REGIONAL (NFS4-R-AU): the two debug overlays that the matched base draws
     here -- sprintf(D_80010044, largestunused()) + FETextRender_FullText, and
     sprintf(D_80010048, AudioMus_Buffered(), AudioMus_Threshold()) +
     FETextRender_FullText -- are absent from the retail regional build (and
     with them the char buffer[32] frame slot: 0x70 -> 0x48). */
  ...
  /* REGIONAL (NFS4-R-AU): retail draws one extra frontend overlay primitive
     after the two-player loop, through the region-only helper at 0x8004DCB4
     (mechanical name kept -- no ground-truth symbol, cf. its own manifest row). */
  func_8004DCB4(0, 8);
