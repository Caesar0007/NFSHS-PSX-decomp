...
/* REGION-ONLY callee reached from the NFS4-R-AU Init_Memcard tail; identity TBD
   ([INFERRED] platform/PAL post-init hook), so it keeps the mechanical name. */
extern "C" void func_800DCED0(void);
...
void Init_Memcard(bool redraw,bool pinkslips)
{
  ...
  /* REGIONAL DELTA (NFS4-R-AU @8003484C): PAL/AU retail selects productLocation 2 where the
     matched base build stores 0 -- oracle `li v0,2; sw v0,16(sp)` vs base `sw zero,16(sp)`. */
  mcrdopts.productLocation = (PRODUCTLOC)2;
  ...
  *(void (** volatile)(void))&mcrdopts.SavingDataProc = SavingProc;
  ...
  padinit();
  timedwait(0x14);
  /* REGIONAL DELTA (NFS4-R-AU @8003484C): retail adds one more no-arg call in the tail,
     after the final timedwait.  The callee is REGION-ONLY (no base counterpart) and its
     identity is TBD, so it keeps the mechanical func_<region_va> name (§8.3).  [INFERRED]
     it is a PAL/platform post-init hook -- its regional body is an empty leaf
...

{
  ...
  /* [branch-polarity fix] oracle's beqz skips the (rare) notitle==true case out-of-line and
     falls straight through the common title path -- arm order flipped to match
     (catalog wave-3 "if/else ARM ORDER controls beqz/bnez polarity" row). */
  /* REGIONAL DELTA (NFS4-R-AU @80034A5C): retail drops the per-player title entirely.
     No PlayerNameExist / PlayerName / TextSys_Word calls survive; the else arm is a
     single sprintf of the literal "NEED FOR SPEED" (oracle a1 = the rodata string at
     region VA 8001168C, read out of NFS4-R-AU.EXE).  `cardnum` ($a1) is consequently
     never read in this build -- the oracle's prologue only latches $a0 and $a2. */
  if (notitle) {
    TITLE[0] = '\0';
  }
  else {
    sprintf(TITLE,"NEED FOR SPEED");
  }
