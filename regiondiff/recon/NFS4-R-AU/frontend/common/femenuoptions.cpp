...

{
  ...
  if (this->currMenu != (tInsideBoxMenu *)0x0) {
    ...
    temp.x = (short)xx;
    /* REGIONAL DELTA (NFS4-R-AU @8001D7CC): the second SetDrawArea clip rect starts 8
       scanlines lower than in the matched base build -- oracle
       `lhu v0,2(t2) ; addu v0,v0,s5 ; addiu v0,v0,8 ; sh v0,42(sp)` (the base build
       stores the bare `addu` result).  It is local to this rect: every other use of
       `yy` aligns instruction-for-instruction, so the +8 belongs to temp.y, not to yy. */
