...
/* REGIONAL (PAL) global, identity TBD -- mechanical name per the no-auto-naming
   rule.  [INFERRED] a LANGUAGE INDEX: one byte at regional VA 0x80115C24 that
   main/Front_InitialMemCardCheck/LoadGame STORE and DoTitleScreen/
   PreLoad__11tScreenMain/GetShapeInfo__15tScreenCongrats/
   DrawForeground__18tScreenTournSelect/UserNameUpperCaseOneLetter READ
   (grep "23588(" over regiondiff/oracles/NFS4-R-AU/).  It has no base-image
   counterpart, so it is NOT resolved through configs/symbol_addrs.txt. */
extern "C" u_char RDAT_80115C24;
...

{
  ...
  if (creditShapeFile[0] == (char *)0x0) {
    /* REGIONAL DELTA: the title art file is per-language --
       "ttl%d.psh" (rodata @0x800127C0 in NFS4-R-AU.EXE) with the
       language byte as its only argument. */
    sprintf(artfilename,"ttl%d.psh",RDAT_80115C24);
...
{
  /* REGIONAL DELTA (PAL): 256-line display instead of 240 (the height CSEs
     with the first call's y=0x100, hence retail's `addu s1,a2,zero`), plus a
     21-line vertical offset written into both DISPENVs' screen rect. */
  SetDefDispEnv(&gEnviro[0].disp,0,0x100,0x200,0x100);
  SetDefDispEnv(&gEnviro[1].disp,0,0,0x200,0x100);
  gEnviro[0].disp.screen.y = 21;
  gEnviro[1].disp.screen.y = 21;
  Draw_InitViews();
  Draw_gRearView[0] = -1;
  ...
  Draw_gPlayer1View[0] =
       Draw_SetView
                 (0,0,0,0x100,0x200,0x100,0,1,10);
  blockclear(&gCView,0x8c);
