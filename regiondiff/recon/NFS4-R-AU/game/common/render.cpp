...

{
  ...
  if (Render_GameSetupWords[3] == 1) {
    /* REGIONAL DELTA (PAL): 256-line screen -- split-screen half height
       0x78 -> 0x80, player-2 y origin 0x178 -> 0x180, full height 0xf0 -> 0x100
       (which then CSEs with the x-origin 0x100 already in $a1). */
    Draw_gPlayer1View = Draw_SetView(0,0x100,0x140,0x100,0x140,0x80,0,1,2000);
    Draw_gPlayer2View = Draw_SetView(0,0x180,0x140,0x180,0x140,0x80,0,1,2000);
  }
  else {
    Draw_gPlayer1View = Draw_SetView(0,0x100,0x140,0x100,0x140,0x100,0,1,3000);
    Draw_gPlayer2View = -1;
  }
  return;
}
...

{
  ...
  Render_CreateViews();
  /* REGIONAL DELTA (PAL): render height 0xf0 -> 0x100. */
  Draw_InitRenderEngine(0x140,0x100,0,0x100,0x140,0x100);
