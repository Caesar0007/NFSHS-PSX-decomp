...
{
  if (gFlip == -1) {
    /* REGIONAL (NFS4-R-AU, PAL): the retail PAL build renders a 256-line frame --
       the height argument is 0x100 (not the NTSC 0xf0), which gcc materializes by
       reusing the y-argument register (oracle `addu a3,a1,zero`). */
    Draw_DirectSetEnvironment(0x140,0x100,0x140,0x100,1,0,1,0,0,0);
    Draw_DirectSetEnvironment(0,0x100,0x140,0x100,1,1,1,0,0,0);
    gFlip = 1;
...

{
  ...
  if (edisplay != 0) {
    ...
    /* REGIONAL (NFS4-R-AU, PAL): PAL display window -- screen.y = 21 scanlines of
       top border, screen.h = 256 active lines (oracle sh 0x15,0x22(sp) / 0x100,0x26(sp);
       the sp+0x18 env buffer's +0xA/+0xE = DISPENV.screen.y/.h). */
    ((DISPENV *)&e)->screen.y = 21;
    ((DISPENV *)&e)->screen.h = 256;
    SetDispMask(0);
...

{
  ...
  /* REGIONAL (NFS4-R-AU, PAL): PAL display window on BOTH flip pages --
     screen.y = 21, screen.h = 256 (oracle sh 0x15,0x22(s0)/0xA(s0);
     sh 0x100,0x26(s0)/0xE(s0), s0 = &gEnviro[0], stride 0x18). */
  gEnviro[1].disp.screen.y = 21;
  gEnviro[0].disp.screen.y = 21;
  gEnviro[1].disp.screen.h = 256;
  gEnviro[0].disp.screen.h = 256;
