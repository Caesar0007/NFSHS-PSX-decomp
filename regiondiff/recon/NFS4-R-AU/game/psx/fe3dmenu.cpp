...

{
  ...
  /* R-AU delta: retail lifts the menu view 8 pixels before the showroom
     override (the oracle's `addiu $s0,$s0,-8` rides the showRoomFlag branch
     delay slot, so it applies on both paths). */
  posY = posY - 8;
  if (showRoomFlag != 0) {
    posY = 0x8c;
  }
  ...
  correct.m[7] = 0;
  correct.m[8] = 0x10000;
  correct.m[4] = 0xad80;   /* R-AU delta: base build uses 0x8980 */
