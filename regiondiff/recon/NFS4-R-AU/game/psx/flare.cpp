...

{
  ...
  if ((sd->head).cprim.PrimPtr < (sd->head).cprim.MPrimPtr + -0x400) {
    ...
    dx = 0x140 - sx;
    dy = 0x100 - sy;   /* REGIONAL (PAL): screen height 256, not 240 */
    flareVis = '\0';
...

{
  ...
  /* REGIONAL (AU/FR-DE/UK-ES-IT/UK-SW = PAL): vertical resolution 256 not 240,
   * so the half/quarter-screen shifts are 0x80/0x40 instead of 0x78/0x3c. */
  pshift = 0x80;
...
gte_stlvnl(&diff);
    if ((diff.vx <= diff.vz) && (-diff.vx <= diff.vz)) {
      *(u_int *)&color = 0x808080;
      ...
      vertRezBy2 = 0x80;   /* REGIONAL PAL: 256/2 */
      posOnScreen.vy = (short)((diff.vy >> 2) + pshift);
      if (FLARE_COMM_MODE == 1) {
        vertRezBy2 = 0x40; /* REGIONAL PAL: 256/4 */
      }
...

{
  ...
  /* REGIONAL (PAL): 256-line screen -> 0x80 / 0x40 (base NTSC 0x78 / 0x3c). */
  pshift = 0x80;
  if (FLARE_COMM_MODE == 1) {
    pshift = 0x40;
  }
