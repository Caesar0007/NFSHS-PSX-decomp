...
/* ===== REGIONAL DELTA (NFS4-R-AU @800E4490, 97 insns vs base 113) =====
 * THREE facts, all source-level; the base body/loop shape is otherwise unchanged:
 *  (1) the splat Y is masked, not divided: retail computes `random() & 0xFF`
 *      (`andi $v0,$v0,0xff`) where the base build does `(u_int)random() % 0xf0`
 *      (the 9-insn multu/mfhi magic-divide chain).  Two sites => -16 insns, which
 *      IS the whole count gap.
 *  (2) ONE vy store, not one per commMode arm: the oracle merges at .L800E458C and
 *      stores `sh $v0,2($s1)` once, from a value computed in either arm.  Writing
 *      it as `vy = ...; q->pos.vy = (short)vy;` (instead of a store inside each arm)
 *      drops `q` from 3 refs to 2 -- and THAT is what flips the local-alloc tie to
 *      retail's handout (walker $s0 / q $s1 / i $s2); with two stores the count is
 *      already exact 97/97 but every walker/q line is an $s0<->$s1 swap (18 diffs).
 *  (3) consequently NO allocation fences are needed here: the base seal's
 *      `__asm__("" : : "r"(i))` (ref-count dial on i) and the `__asm__("" : "=r"(q)
 *      : "0"(q))` launder are both DELETED -- this candidate contains no asm and no
 *      volatile.  `q` itself must stay: dropping it loses retail's `move $s1,$s0`
 *      copy in the jal delay slot (95 insns, 46 diffs).
 * MEASURED THIS ROW: seed 34 @113 | andi only 18 @97 (pure s0<->s1) | + i-fence
 * dropped 18 (inert) | decl-order swap 18 | q block-local 43 @96 | q before the vx
 * store 71 @102 | q carrying startTick 16 (wrong: routes a walker store through q)
 * | one-store + launder PASS | one-store + (long long) launder PASS | one-store, no
 * asm at all **PASS 97/97** (adopted) | one-store with no q 46 @95.
 * ===================================================================== */
  int i;
  u_int vy;
      ...
      if (WEATHER_GAME_TICKS >= splats[i].startTick) {
        if (splats[i].startTick + 0x20 < WEATHER_GAME_TICKS) {
          if ((num < gCurrentNumSplats) && (i == gCurrentNumSplats + -1)) {
            ...
            q = &splats[i];
            if (WEATHER_GAMESETUP_COMM_MODE == 1) {
              vy = ((u_int)random() & 0xff) >> 1;
            }
            else {
              vy = (u_int)random() & 0xff;
            }
            q->pos.vy = (short)vy;
