...

{
  ...
  /* REGIONAL DELTA (NFS4-R-JPN @800136AC): the whole on-screen DEBUG OVERLAY block is
     absent from retail -- the two sprintf/FETextRender_FullText pairs that print
     largestunused() and AudioMus_Buffered()/AudioMus_Threshold(), together with the
     `char buffer[32]` they filled.  The oracle goes straight from
     `jal func_800BEB98` (Draw_StartRenderingView) to `jal func_800BE970`
     (Draw_GetDRAWENV) and its frame is -72 vs the base build's -112 (the 32-byte
     buffer plus its alignment).  Base MATCH note kept below for the record:
       "retail keeps these adjacent format strings as distinct symbols; expressing
        both as bigBuf offsets lets GCC common their base into a long-lived saved
        register (152 diffs)" -- moot here, both strings are unreferenced in JPN. */
  ...
  this->DrawHelpIcons();
  /* REGIONAL DELTA (NFS4-R-JPN): retail widens the two-player-race gate with the two
     pad-ID '#' tests the base build only spends in DrawHelpIcons -- oracle
     `lbu 4(v1);bnez / lbu 36(v1);bnez / lbu 5(v1);li a0,35;beq / lbu 37(v1);bne`
     off ONE shared `addiu v1,v0,-1648` gPadinfo base. */
