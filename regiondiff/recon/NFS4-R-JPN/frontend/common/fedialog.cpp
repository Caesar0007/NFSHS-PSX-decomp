...

{
  ...
  /* R-JPN delta: every help text ID is shifted +1 relative to the base
     build (one extra entry earlier in the Japanese string table). */
  ...
  /* R-JPN delta: no two-controller widening -- the Japanese build always uses
     the 0x28 text inset (the `helpcontrollers == 3` arm and its 0x46 inset are
     absent from the retail JPN code). */
  this->lefttext = 0x28;
...

{
  ...
  if (firstTick + 0x32 <= ticks) {
    ...
    __asm__("" : : "r"(ticks));
    /* R-JPN: the shorter body lets gcc rotate a plain `for`; retail keeps the
       top test + unconditional `j` back-edge, so spell the loop
       exit-in-the-middle (catalog B / methodology 3.12 #15a inverse). */
    while (1) {
      ...
      /* R-JPN delta: the whole `helpcontrollers` fan-out (pad-35 / special
       * two-button / 0xa0-0x50-0x40 arms) is absent from the Japanese build --
       * only the plain pad-65 arm survives.  With the single use left, gcc
       * narrows the `int cont[]` load + `(u_short)` mask into the oracle's
       * one `lhu 176(v0)`. */
      if (i > 0) {
        FeTools_DrawPSXButton(0x41,(u_short)this->cont[i],this->left + 0x14,
