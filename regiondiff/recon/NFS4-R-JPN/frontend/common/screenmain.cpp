  ...
  if ((((this->fVideoShapes[this->fCurrentSlot].async_handle == 0) &&
    ...
    if (num == -1) {
      do {
        ...
        /* REGION JPN: 29 backgrounds (base build has 28). */
        this->fCurrentBG[this->fCurrentSlot] = rand() % 0x1d;
...

{
  ...
  /* REGION JPN: retail folds the PreLoad shape/background setup into
     Initialize (29 backgrounds, base build has 28) and turns the shape-wait
     do-while into a while whose flag is initialized up front. */
  shapesLoaded = false;
  ...
  this->fCurrentBG[0] = rand() % 0x1d;
  i = 0;
  this->fCurrentBG[1] = (this->fCurrentBG[0] + rand() % 0x1c + 1) % 0x1d;
  do {
    sprintf(buffer,"zyVid%02d",this->fCurrentBG[i]);
    ::AsyncLoadShapeFile((tScreen *)this,buffer,this->fVideoShapes + i);
    i = i + 1;
