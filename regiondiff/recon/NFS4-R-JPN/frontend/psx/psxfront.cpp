...
{
  /* R-JPN: the per-view draw-memory budget is 86400 bytes, not 80000
     (all three sites shift together; 0x13880 -> 0x15180). */
  Draw_InitViewOT();
  gEnviro[0].server = (char *)reservememadr("ps0",86400,0x10);
  gEnviro[1].server = (char *)reservememadr("ps1",86400,0x10);
  Draw_SetViewMemBudget(Draw_gPlayer1View[0],86400);
  return;
}
