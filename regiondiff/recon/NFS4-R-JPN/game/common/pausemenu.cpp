...
{
  /* REGIONAL DELTA (NFS4-R-JPN @800A82E4, 44 insns): the Japanese build draws NO
     left/right arrow prims -- the whole `selected && language == 0` GT4 block is
     absent, and with it the x/y locals (the second word x is consumed inline by
     the call).  Everything else is the base body verbatim. */
  short index;
  int vtable_p;
...

{
  ...
  while (i < 15) {
    ...
    /* REGIONAL DELTA (NFS4-R-JPN @800A8534, 96 insns vs base 169): as in
       tPMenuItemLeftRightChoice::Draw the Japanese build has NO arrow prims --
       the `selected && language == 0` GT4 block is gone.  With it go the base
       seal's two allocation fences: the register pressure they were shaping came
       from that block (retail base uses 10 saved regs here, the JPN build 6). */
