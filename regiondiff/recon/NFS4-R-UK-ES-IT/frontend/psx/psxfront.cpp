...

{
  ...
  if ((u_char)frontEnd.language == 0xff) {   /* sentinel: -1 == 0xFF as unsigned byte (this build's char is unsigned -> lbu;bne 0xFF) */
    ...
    selected = 0;
    sprintf(fullName,STR_FRMT[0],Paths_Paths[0x20],"lang.psh");
    ...
    while ((key != 2) && (key != 0x2000)) {
      ...
      while (i < 3) {
        ...
        }
        {
          ...
          movfxya(*(shapetbl **)((char *)shapes + selOffset),x + centerOffset,y);
        }
        shapeOffset = shapeOffset + 4;
        i = i + 1;
        nextX = x + 10;
        x = nextX + shapes[7]->width;
        }
      }
