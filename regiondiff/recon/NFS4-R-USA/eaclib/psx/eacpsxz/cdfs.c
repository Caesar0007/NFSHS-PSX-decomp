...
extern int CD_systaskfunc(void)
{
    ...
    case 2: {                              /* CdlComplete -> a disc settled */
        ...
        if (t < 2) {                      /* disc type 1 or 2 == a usable disc */
            CD_timeout = 0;
            done = 1;
        }
        break;
    }
    case 5:                                /* CdlDiskError -> expire the watchdog deadline */
        ...
        if (CD_timeout == 0) {
            CD_timeout = ticks[0] + timerhz[0] * 5;
        } else if (ticks[0] >= CD_timeout) {
            CD_timeout = 0;
            done = 1;
        }
        break;
    ...
    if (done) {
        ...
        if ((ctx->info & 1) != 0) {       /* a transfer was in progress -> resume it */
            ctx->ringIdx = 0;
            ctx->timeout = timerhz[0] * 12;
