...

{
  ...
  /* REGIONAL DELTA (NFS4-R-USA @80013894): retail DROPS the two debug overlay
     lines the base build draws here --
     ...
     -- which is the whole 33-instruction shortfall (360 vs the base's 393), and
     with them the `char buffer[32]` scratch. */
...
/* REGIONAL DELTA declarations (NFS4-R-*): RunDemoVideo blanks VRAM around the
   movie.  Callee identity by masked content-match of the regional slice against
   the base image: 800EE35C = ClearImage (base 800EDA00), 800EE1D8 = DrawSync
   (base 800ED87C).  Spelled exactly as the sibling FE owners do
   (screentracks/screenpinkslips externs). */
extern "C" {
...

{
  ...
  char buffer [40];
  /* REGIONAL DELTA (NFS4-R-USA @800140E8): the whole VRAM rectangle retail
     blanks either side of the movie. */
  RECT vramRect;
  ...
  if ((tMenuNFS4 *)this->fCurrentMenu[0] == &menuDefs[0]->menuMain) {
    ...
    /* REGIONAL DELTA: clear the full 1024x512 frame buffer + wait for the
       GPU both before and after the movie, and play the CURRENT index
       (retail's `lbu` of currentVideo; base plays currentVideo + 1). */
    vramRect.x = 0;
    vramRect.y = 0;
    vramRect.w = 1024;
    vramRect.h = 512;
    ...
    DrawSync(0);
    play_movie((u_char)currentVideo);
    vramRect.x = 0;
    vramRect.y = 0;
    vramRect.w = 1024;
    vramRect.h = 512;
    ...
    LoadAllHelpShapes();
    /* REGIONAL DELTA: retail moves the UpdateMusic/AudioMus_Volume pair to the
       very END of the function (after TransitionOn and the currentVideo
       update); base runs them here, before the largestunused block. */
    {
    ...
    /* REGIONAL DELTA: retail alternates between two clips (5 - n) instead of
       the base's 3-clip `(n + 1) % 3` magic-divide cycle. */
    currentVideo = 5 - currentVideo;
    this->UpdateMusic();
    AudioMus_Volume((int)((u_int)(u_char)frontEnd.musicVolume * 0x23) >> 6);
  }
  return;
