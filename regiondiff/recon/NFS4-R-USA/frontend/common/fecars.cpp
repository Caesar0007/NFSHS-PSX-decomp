...

{
  ...
  for (i = 0; i < this->fNumCars; i++) {
    if ((int)(signed char)this->fCars[i].fCarID == (int)carID) {
      /* REGIONAL DELTA (NFS4-R-USA @80016374): retail refreshes the entry's
         fAvailable from the manager's per-model availability table before
         handing the record out.  The byte STORE inside the loop is what
         kills the base build's LICM of fNumCars/fCars: retail reloads both
         every iteration and strength-reduces fCars[i] to a byte-offset giv
         in $a1 (this stays live in $a0).
         MATCH: index with the PARAM carID, not with the just-loaded
         fCars[i].fCarID -- cse substitutes the compared register on the
         equal path (oracle's `addu v0,a0,v0` uses the lb result), while a
         second source-level use of the field gives the load TWO uses and
         combine no longer folds lbu+sll24+sra24 into `lb` (30 insns/12
         diffs vs 28/PASS). */
      this->fCars[i].fAvailable =
          this->fAvailableCars[carID];
      return &this->fCars[i];
    }
  }
  return (tCarInfo *)0x0;
...

{
  ...
  if ((this->fCarListFilter & 0x89U) != 0) {
    ...
    /* REGIONAL DELTA: retail sets firstCar INSIDE this arm (the other two set
       it from fNumCars), so gcc emits it as a copy of the zero already held
       for carInfo and the `j` delay slot carries that copy; a function-scope
       `firstCar = 0;` before the chain leaves the slot to a duplicated
       `sll v0,s3,16` (381 insns vs the oracle's 380). */
    firstCar = 0;
  }
  ...
  if (firstCar < lastCar) {
    /* REGIONAL DELTA (NFS4-R-USA @80017BE4): retail takes the
       oldValue/carInfo/oldCountry snapshot HERE -- after the two clamps and
       inside the firstCar<lastCar guard -- where base takes it at the top of
       the function, before the list-filter chain. */
