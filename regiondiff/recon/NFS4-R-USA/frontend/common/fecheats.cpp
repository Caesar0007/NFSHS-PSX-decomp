...
/* REGIONAL: this owner also reaches tCarManager::SetCarAvailable, which the
   base owner's filtered type surface does not declare (fecheats_types.h adds
   "only the one car-manager member used by this owner").  Free-function view
   bound to the cfront symbol -- `this` rides $a0 exactly like the method, so
   this is byte-neutral (methodology 3.23b). */
void CarManager_SetCarAvailable(tCarManager *mgr, int carModel, bool avail)
    asm("SetCarAvailable__11tCarManager10tCarModelsb");
...
{
  /* REGIONAL DELTA (retail): the whole 31-case effect switch is REPLACED by an
     11-case one.  0..7 keep the car-class unlock; 8/9/10 are three
     tCarManager::SetCarAvailable calls (oracle func_80016F34 sits exactly 0x84
     after func_80016EB0 == SetClassAvailable, the same delta as the base image's
     SetCarAvailable@0x80016DEC - SetClassAvailable@0x80016D68, and both take the
     same object base 0x8011552C = carManager in the regional layout).
     [INFERRED] the regional cheat set is 11 entries: 8 car classes + 3 bonus
     cars (models 19/20/28); every base cheat from AllCars upward is gone, so
     the enumerator NAMES for 8/9/10 no longer carry their base meaning and are
     spelled numerically here.  gcc cross-jumps the three SetCarAvailable calls
     into one shared jal (oracle .L80023418). */
  switch(cheat) {
  case cheat_Roadster:
  case cheat_Pony:
  case cheat_Saloon:
  ...
  case cheat_CopCar:
    ...
    break;
  case 8:
    CarManager_SetCarAvailable(&carManager,19,true);
    break;
  case 9:
    CarManager_SetCarAvailable(&carManager,20,true);
    break;
  case 10:
    CarManager_SetCarAvailable(&carManager,28,true);
    break;
  }
  return;
}
...

{
  ...
  i = 0;
  while (true) {
    if (2 < i) break;   /* REGIONAL: cheatList shrinks 10 -> 3 */
    for (j = 0; j < 8; j = j + 1) {
    ...
    if (j == 8) {
      ...
      pcVar4 = TextSys_Word(0x27b);   /* REGIONAL: word id 634 -> 635 */
      ptVar2 = FEApp;
...

{
  ...
  do {
    ...
    i = i + cheat_Pony;
  } while (i < 0xf);   /* REGIONAL: 31 -> 15 cheat bits */
  return;
}
...

{
  ...
  do {
    ...
    i = i + 1;
  } while (i < 2);   /* REGIONAL: bonusList shrinks 3 -> 2 */
  return result;
}
