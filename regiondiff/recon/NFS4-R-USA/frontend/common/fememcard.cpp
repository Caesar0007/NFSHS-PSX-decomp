...
/* REGION-ONLY nullsub added by the retail build (its own slice
   regiondiff/oracles/NFS4-R-USA/func_800DCEA4.s is `jr $ra; nop`).  Callers in
   this TU: Init_Memcard, DeInit_Memcard, SaveGame x3; also Movie_Play and
   LoadGame elsewhere -- so it lives outside this object and is declared, not
   defined, here.  Mechanical name per the README's no-auto-naming rule. */
extern "C" void func_800DCEA4(void);

/* REGIONAL-LAYOUT GLOBAL, identity TBD.  Read as a WORD at region VA 0x8013E560
   (4 bytes past MEMCARDFRONTENDISINITTED's regional address 0x8013E55C, whose
   identity IS fixed by DeInit_Memcard's base body).  The base symbol that the
   bracketed delta 0x12D8 lands on is `gCheckTotalTime`, which is a CHAR -- so
   this is NOT that symbol; it is either a retail-added global or the far side of
   a bss shift, and this agent could not settle it without a per-region link map.
   Mechanical name per the README's no-auto-naming rule.
   [INFERRED] ROLE: guards restoring frontEnd.language after a card load, i.e. a
   "keep the currently selected language" flag -- consistent with the localized
   retail releases the regional corpus exists to document.  The gate cannot
   confirm the identity either way: the reference is a masked %hi/%lo pair. */
extern int RDAT_8013E560;
...
{
  return Confirm(CURRENTPLAYER + 0x324,0x290);
}
...
{
  return Confirm(CURRENTPLAYER + 0x328,0x291);
}
...
void Init_Memcard(bool redraw,bool pinkslips)
{
  ...
  padinit();
  timedwait(0x14);
  /* REGIONAL DELTA (NFS4-R-*): retail calls an extra NULLSUB here.  Its own
     regional slice (regiondiff/oracles/NFS4-R-USA/func_800DCEA4.s) is a bare
     `jr $ra; nop`, i.e. a release-compiled-out debug/log hook -- REGION-ONLY,
     so it keeps the mechanical func_<region_va> name (README no-auto-naming).
     Every call site passes NO argument (`jal; nop`, no $aN setup), so the
     signature really is void(void) -- methodology 3.2's "nullsub still takes
     its real args" caveat does not bite here. */
  func_800DCEA4();
  return;
}
...
void DeInit_Memcard(void)
{
  ...
  padinit();
  func_800DCEA4();   /* REGIONAL DELTA: the same added nullsub (see Init_Memcard) */
...

{
  ...
  WarningDialog.string = TextSys_Word(player + 0x277);
...

{
  ...
  /* REGIONAL DELTA: retail snapshots frontEnd.language BEFORE the card image
     overwrites the whole frontEnd block, and restores it at the very end when
     RDAT_8013E560 is set (see the tail).  Oracle: `lbu $v0,%lo(frontEnd+0x340)`
     spilled to the frame at sp+5776, `sb` back from there at 0x80035538.
     DECL POSITION IS LOAD-BEARING: declared first, it takes the LOWER of the two
     spill slots (5776) and `count` takes 5780, as retail has them. */
  int savedLanguage;
  short cardNum;
  bool finished;
  int result;
  ...
  CURRENTPLAYER = player;
  CURRENTLYUSINGMEMCARD_arr[0] = 1;
  savedLanguage = frontEnd.language;
  tDialogNoInputMessage WarningDialog;
  MCRDFILE_def memCardFile;
  ...
  do {
  /* REGIONAL DELTA: retail calls MakeWayForMemoryCard() BEFORE bumping the retry
     counter (m2c loop_11: func_80034520(); sp1694 += 1; func_800349E0(...)).  The
     base object has the two statements the other way round. */
  MakeWayForMemoryCard();
  count = count + 1;
  ...
  while (true) {
    ...
    case 0xf:
      if (VerifySuccessfulRead(&memCardData)) {
        ...
        returnmessage = 0x28e;
        SetPads();
        /* REGIONAL DELTA: retail does NOT share the late `finished = true` tail
           with the success arm any more -- m2c shows the success arm ending in
           its OWN `var_s1 = 1` while only the two failure paths meet at
           block_35.  (The base object's `goto finish_card_event;` here is what
           kept `finished`'s ref count high enough to win $s1; with the shared
           edge removed retail's allocation falls out.) */
        finished = true;
      }
      else {
        /* REGIONAL DELTA: retail REPLACED the base object's "show the read-failed
           message dialog" arm with the SAME card-status classification the
           2/3/7/a/b/10/17 arm runs (m2c renders them sharing the block_31 /
...
finish_card_event:
  ...
  /* REGIONAL DELTA: the retail tail also invalidates the memcard screen's icon
     cache (the base body only does this in SaveGame) and restores the language
     snapshot taken at entry.  The `screenMemcard->fGetNewIcons = 1` store sits
     in the guard's branch delay slot, i.e. it is UNCONDITIONAL and precedes the
     `if` in source. */
  screenMemcard->fGetNewIcons = 1;
  if (RDAT_8013E560 != 0) {
    frontEnd.language = (char)savedLanguage;
  }
