...
/* [NFS4-R-USA REGIONAL DELTA] five source-level changes vs the base body:
   (1) every TextSys word id is +1 (0x2c0->0x2c1, 0x321/0x322->0x322/0x323);
   (2) the dialog is a tDialogYesNoTri (retail emits the base __12tDialogYesNo
       ctor call followed by the inline `_vf = &tDialogYesNoTri_vtable` store
       at +0x60 -- the same shape base MenuExtended_GoToTwoPlayerSingleRace has);
   (3) `CURRENTPLAYER = screenMemcard->player;` (regional 0x80051EF0 ==
       menuDefs+0x10 == base CURRENTPLAYER @0x80051a68);
   (4) the Run() result is tested `== 1`, not `!= 0`;
   (5) a new else-arm hides the no-input memcard dialog. */
extern int CURRENTPLAYER;   /* @base 0x80051a68 */
...

{
  ...
  tMenuCommand();
  CURRENTPLAYER = screenMemcard->player;
  /* SYM-INLINE-THIS: SetChoices */
  AreYouSure.SetChoices(0x322, 0x323, 0)->string = TextSys_Word(0x2c1);
  if (AreYouSure.Run() == 1) {
    GenericMenuLoadGame((int)screenMemcard->player);
  }
  else {
    ((tDialogBase *)&FEApp->MemCardDialog)->Hide();
  }
  return;
}
...
/* [NFS4-R-USA REGIONAL DELTA] callees of the retail-only music-buffering wait
   loop at the tail of MenuExtended_AwardPinkSlipsCar.  Identified by
   region->base VA delta bracketing against oracles/INDEX.tsv (all four sit in
   the +0x128 / +0xA6C / +0x160 / +0x4E8 delta bands of their neighbours) and by
   the identical call shape in base tScreen::DisplayLoadingText.
   Signatures copied verbatim from fescreen_externs.h. */
int  AudioMus_Buffered(void);    /* base @0x80079FDC -> region 0x8007AA48 */
int  AudioMus_Threshold(void);   /* base @0x80079F58 -> region 0x8007A9C4 */
void FeAudio_systemtask(int);    /* base @0x80015948 -> region 0x80015A70 */
...

{
  ...
  dlgThis2->yesnowords[0] = 0x292;
  dlgThis2->yesnowords[1] = 0x293;
  dlgThis2->fDefault = 1;
  fWinner = screenPinkSlipCongrats->fWinner;
  mess = TextSys_Word(0x29b);
  ...
  DeInit_Memcard();
  ((tDialogBase *)&FEApp->NoInputMemCardDialog)->Hide();
  /* [NFS4-R-USA REGIONAL DELTA] retail waits for the music stream to refill
     before leaving, redrawing the "loading" text every frame (the same
     word/coords/justify pair tScreen::DisplayLoadingText uses, word id +1). */
...
tGlobalMenuDefs::tGlobalMenuDefs()
 ...
 , menuTwoPlayer(0x1004, (tScreen *)screenMain, (tMenu *)0x0, (tMenu *)0x0, 0, 0xb6, (tMenuItem *)&itemTwoPlayerTestDrive, &itemTwoPlayerDuel, &itemTwoPlayerHotPursuit, &itemTwoPlayerPinkSlips, 0)   /* +0x920 tMenuNFS4 */
 ...
 , menuSingleCarSelect(({ tMenuItem *garageCarItem = &itemGarageCar; (void)garageCarItem; 0x1a00; }), (tScreen *)screenCarSelect, (tMenu *)0x0, (tMenu *)&menuCarOptions, (void (*)(tMenuCommand&))MenuExtended_GoToRace, 0xbb, (tMenuItem *)&itemCarSelectRace, &itemCar, &itemColor, &itemShowcase, 0)   /* +0x1250 tMenuNFS4 */
 ...
 , menuCarGarage(0x1a00, (tScreen *)screenCarSelect, (tMenu *)0x0, (tMenu *)&menuCarOptions, (void (*)(tMenuCommand&))MenuExtended_GoToRace, 0x90, (tMenuItem *)&itemCarSelectRace, &itemGarageCar, &itemCarDealer, &itemUpgradeCar, 0)   /* +0x1368 tMenuNFS4 */
 , menuPostCarGarage(0x1a00, (tScreen *)screenCarSelect, (tMenu *)0x0, (tMenu *)&menuCarOptions, (void (*)(tMenuCommand&))MenuExtended_GoToRace, 0x90, (tMenuItem *)&itemCarSelectRace, &itemUpgradeCar, 0)   /* +0x13E4 tMenuNFS4 */
 ...
 , menuGoToCarDealer(0x2200, (tScreen *)screenCarSelect, (tMenu *)0x0, (tMenu *)0x0, 0, 0x91, (tMenuItem *)&itemGoToBuyCar, &itemGoToSellCar, 0)   /* +0x1BE8 tMenuNFS4 */
 ...
 , menuCarDealer(0x2240, (tScreen *)screenCarSelect, (tMenu *)0x0, (tMenu *)0x0, 0, 0x91, (tMenuItem *)&itemDealerCar, &itemDealerColor, &itemBuyCar, &itemDealerShowroom, 0)   /* +0x1D48 tMenuNFS4 */
 ...
 , menuCarSeller(0x2200, (tScreen *)screenCarSelect, (tMenu *)0x0, (tMenu *)0x0, 0, 0x91, (tMenuItem *)&itemSellerCar, &itemSellCar, 0)   /* +0x1E34 tMenuNFS4 */
 ...
 , menuCarUpgrades(0x2200, (tScreen *)screenCarSelect, (tMenu *)0x0, (tMenu *)0x0, 0, 0x92, (tMenuItem *)&itemPurchaseUpgrade1, &itemPurchaseUpgrade2, &itemPurchaseUpgrade3, 0)   /* +0x1F34 tMenuNFS4 */
