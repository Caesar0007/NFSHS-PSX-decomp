...

{
  ...
  if (0 < flare_intensity) {
    /* REGIONAL DELTA (NFS4-R-USA @8001C100): the flare's text-word index is one
       higher than the base build's (0x1df vs 0x1de) -- retail inserted one entry
       ahead of it in the string table.  Sole word-level delta in this function. */
    x = TextSys_WordX(0x1df);
  ...
  : tMenuItem(textDescription)
{
  ...
  fDiffY = diffy;
  fFillback = fillback;
  /* LAW 05A (the SLD is the statement order): the SLD attributes a SECOND
     `fFlags |= 0x80` to line 624, the ctor's closing line -- retail really ORs
     the bit in twice.  Both are LOAD-BEARING, re-measured 2026-08-30: dropping
     the first gates 13, dropping the second 37, and either way the body is
     39 insns against retail's 42.  Do not "clean up" the duplicate. */
  fFlags |= 0x80;
}
...

{
  ...
  if (this->fFadeVal != 0x80) {
    ...
    this->MyLeftRightDraw((short)x,(short)y);
    /* REGIONAL DELTA (NFS4-R-USA @8001E4E4): the ON/OFF text-word pair is one
       higher than the base build's (0x67/0x68 vs 0x66/0x67) -- the retail
       string-table insertion.  Same pair shifts in DrawOneSong below. */
    FETextRender_FullTextRGB(TextSys_Word(0x67),(short)(x + 0x37),(short)y,ColTextOn,'\0',0);
    FETextRender_FullTextRGB(TextSys_Word(0x68),(short)(x + 0x9e),(short)y,ColTextOff,'\0',0);
  }
}
...

{
  ...
  /* REGIONAL DELTA (NFS4-R-USA @8001EDA8): the ON/OFF text-word pair is one higher
     than the base build's (0x67/0x68 vs 0x66/0x67) -- the retail string-table
     insertion, same pair as tMenuItemOnOffLeftRightChoice::Draw. */
  FETextRender_FullTextRGB(TextSys_Word(0x67),
             (short)(x + (u_short)w - 0x5a),y + 6,ColTextOn,'\0',2);
  FETextRender_FullTextRGB(TextSys_Word(0x68),
...

{
  ...
  /* W85-S3 (2026-09-02): `u_int` (was `u_char`) -- the classic methodology 3.12 #9
     lever.  A u_char local is re-masked on every use, so gcc hoisted an
     `andi a0,a1,255` in FRONT of the `(selectedChar - 0x23) < 2` test where retail
     compares the raw byte and issues the `andi` only in the else arm.  The
     `__asm__("" : "+r"(selectedChar))` byte-extension fence that used to sit in the
     else arm is DELETED: with the u_int type the mask lands exactly where retail has
     it.  Falsified with the fence removed and the type left u_char (all 4 diffs
     @240/240): plain `& 0xff`, an explicit `(u_int)(u_char)` cast, and an `(int)`
     cast inside the compare; a fresh re-read for `character` 14, a do{}while(0) dial
     on it 11. */
  u_int selectedChar;
  short column;
  u_int stringLength;
...

{
  ...
  if (this->fFadeVal != 0x80) {
    ...
    /* W85-S3: the `__asm__("" : : "r"(row))` and `__asm__("" : "=r"(columnx) :
       "0"(columnx))` fences that used to bracket the two statements below are BOTH
       DELETED.  Removing either ALONE regresses (3 / 10 diffs -- the row fence alone
       leaves an extra load-delay `nop` because sched2 puts `addiu s1,a2,258` before
       the `lh v1,36(s2)` instead of into its delay slot), but removing the PAIR is
       exact 254/254.  Falsified: xx computed last 18, xx last with the columnx fence
...

{
  ...
  for (i = 0; i < 10; i++) {
    /* REGIONAL DELTA (NFS4-R-USA @800206E0): the row-label text-word base is one
       higher than the base build's (0x1fc vs 0x1fb) -- the retail string-table
       insertion (tScreenUserName::Initialize carries the same shifted base). */
    sprintf(this->fRowList[i],TextSys_Word(i + 0x1fc));
  }
...

{
  ...
  /* W85-S3 (2026-09-02): the volatile STORE barrier below is KEPT and re-certified.
     ...
     present).  Falsified this wave: plain store 29; the two reads sourced from sVar2
     instead of memory 29; a do{}while(0) dial on the store 29; the volatile moved from
     the store onto the two READS 7 (best alternative, still not exact -- exactly the
     lhu + sll/sra degradation the note below predicts).  It is a genuine store->reload
     barrier (methodology 3.12 #13 family), not a coloring crutch.
...
fEnableStore:
  ...
  /* W85-S3: the `__asm__("" : : "r"(sv))` 05C read-only fence that used to sit
     here ("extends sv's live range past v's death so local_alloc hands v the lower
     reg") is DELETED -- it is completely INERT; the whole TU gates 92/92 with and
     without it.  NOTE the interaction: with the fence present, dropping the
     `*(volatile short *)&this->fEnableVal` store barrier above cost 5 diffs; without
     it, 29.  The store barrier itself is kept (see its own note). */
