...

{
  ...
  if (_7tScreen_fSuppressLoadingText != 0) {                              /* 37 */
    ...
    for (i = 0; i < 2; i = i + 1) {                                       /* 48 */
      ...
      /* REGIONAL (R-USA, W86-B8): +1 retail string-table shift on the
       * "loading" text id (0x27d -> 0x27e; AUDIT_LO16 insn 30 and 40).
       * 32B-5 check: the id is spelled TWICE in one loop body, but the
       * intervening call clobbers $a0 so retail emits TWO `li $a0,638` —
       * both source sites must move together (the gate's 2 diff pairs). */
      FETextRender_MenuTextPositionedJustify(0x27e,0x1e0,0xdc,1,textState_Selected,textType_ScreenInfo); /* 59 */
      PSXDrawSquare(0,0x1e0,0xdc,-textpixels(TextSys_Word(0x27e)) - 5,7); /* 60 */
