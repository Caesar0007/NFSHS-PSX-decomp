...
struct RUSA_GameSetupAudioModeView {
  unsigned char _beforeAudioMode[0xec];
  int audioMode;
};
...
int          gPSXMemCardFull;   /* @0x80051910; base SYM 5984a8: INT */
int          colourChosen[8];   /* @0x80051914  (bss(zero)) */
/* P871: base SYM 5984e6/59850d are scalar PTR STRUCT and BOOL, not arrays.
   Keep the shared scalar declarations and regional byte behavior together. */
tAllScreens  *gAllScreens;   /* @0x80051934; one pointer, 4 B (base VA) */
bool         memCardReadOK;   /* @0x80051938; SYM BOOL (base VA) */
...

{
  ...
  if (frontEnd.raceType == RaceType_HotPursuit) {
    ...
    missionManager.LoadDescription((0 < streamData.numTraffic) ||
                                   (frontEnd.gameMode == 1));           /* 1572 */
...

{
  ...
  if (frontEnd.raceType == RaceType_Tournament) {
    ...
    if ((frontEnd.carListType == '\x01') ||
        ...
        (frontEnd.raceType == RaceType_PinkSlips)) {
...

{
  ...
  frontEnd.audioMode = (char)((RUSA_GameSetupAudioModeView *)&GameSetup_gData)->audioMode;
  i = 0;
  do {
