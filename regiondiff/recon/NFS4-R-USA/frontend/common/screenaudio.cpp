...
DrawFgAudio_fadeDone:
  ...
  if (99 < fade) {
    /* REGIONAL (all 6 retail regionals): +1 text-id shift -- the retail string
     * table gained one entry ahead of this id, so 0x27d becomes 0x27e at BOTH
     * uses (they are separate `li $a0` insns, not one CSE-shared constant --
     * AUDIT_LO16 insn 41 and insn 51, 2404027d -> 2404027e each). */
...
DrawBg_noSlider:
  ...
  if ((-1 < percent) || (-1 < lastpercentage)) {
    ...
    if (perfade != 0x80) {
      /* REGIONAL (all 6 retail regionals): +1 text-id shift, both uses
       * (AUDIT_LO16 insn 118 and insn 119, 240401dc -> 240401dd each). */
      FETextRender_FullTextRGB(sBuildOutput,(short)TextSys_WordX(0x1dd),
                               (short)TextSys_WordY(0x1dd),ColText,'\0',1);
    }
  }
  {
    int i;
