...
{
  /* REGIONAL DELTA (NFS4-R-USA @800388C0, 39 insns vs base 131): retail's PreLoad
     stops right after the base-class PreLoad -- the shape-array init, the two
     background draws and the async load loop all moved into Initialize (see
     below).  Everything up to tScreen::PreLoad() is byte-for-byte the base body. */
...

{
  ...
  /* REGIONAL DELTA (NFS4-R-USA @8003895C, 221 insns vs base 129): the tail of the
     base PreLoad (shape-array init loop, the two background picks, the async load
     loop) runs HERE, right after the base-class Initialize.  `shapesLoaded` is
     declared/zeroed at the top of the function and the wait loop is a `while`, not
     a `do`: the oracle zeroes $s4 in the prologue (and hands the same zero to the
     first loop counter, `move s2,s4`) and guards the wait loop with `bnez s4` --
     the un-folded rotation guard m2c prints as `if (0 == 0)`.  cse doesn't reach
     across the intervening calls, so the always-false test survives. */
  bool shapesLoaded = false;
  short i;
  char buffer [32];

  this->tScreen::Initialize();
  /* MATCH: ONE fn-scope `short i` serves every loop -- splitting it costs the
     allocno its s2 handout.  A plain `short` counter reproduces the oracle's
     per-use sll/sra + `addiu v0,i,1; addu i,v0,zero` bump. */
  i = 0;
  do {
