...
/* REGIONAL DELTA declarations (NFS4-R-*): the retail DrawForeground also
   renders the "do not remove the memory card" warning line.  Callee identity
   recovered by masked content-match of the regional slice against the base
   image: 800212E0 = FETextRender_MenuTextPositionedJustify (base 80021180),
   800B9F18 = TextSys_Word (base 800B9250), 800E4A2C = textpixels (base
   800E3E20), 8004F12C = PSXDrawSquare (base 8004EC44) -- the last two of
   which screenmemcard_externs.h already carries. */
...
{
  /* SYM-CODEGEN-CARRIER (regional): fadeRaw -- the UNCLAMPED doubled value
     retail keeps in $a1 for the new guard test. */
  int fadeRaw;
...
/* REGIONAL DELTA declaration (NFS4-R-*): retail's Cleanup drains the music
   stream before leaving the screen.  Base's screenmemcard_externs.h does not
   carry this prototype (the base Cleanup never calls it); other frontend
   owners spell it exactly this way (fescreen/screenmain/feapp externs). */
...

{
  ...
  /* REGIONAL DELTA (NFS4-R-USA @80048230): retail spins the frontend audio
     task until the music stream is buffered past its threshold before the
     screen goes away.  Callee identity recovered by masked content-match of
     the regional slice against the base image: 8007AA48 = AudioMus_Buffered
     (base 80079FDC), 8007A9C4 = AudioMus_Threshold (base 80079F58),
     80015A70 = FeAudio_systemtask (base 80015948). */
