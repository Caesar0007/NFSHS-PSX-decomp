...

{
  ...
  /* REGIONAL (all 6 retail regionals): +1 text-id shift -- 0x86/0x87/0x88
   * become 0x87/0x88/0x89 (AUDIT_LO16 insns 24/32/41).  The three ids are one
   * `li` each (distinct switch arms), so no CSE sharing. */
  switch (numplaces) {
  case 3:
    sprintf(string1,TextSys_Word(0x87),0);
    break;
  case 6:
    sprintf(string1,TextSys_Word(0x88),0,0);
    break;
  case 9:
    sprintf(string1,TextSys_Word(0x89),0,0,0);
    break;
  }
...
/* MATCH W61-A17 (127 -> 95 diffs, 556/561 insns), continued to 56/559:
   ...
   so brdist/branch-word census is unaffected by construction.)  This is retained
   only as a historical diagnostic; post-compilation text moves are not permitted.

   ==== W79 source-only (2026-08-26): 4 @561 -> 3 @562. ====
   Remove the non-SYM `self` local and use the implicit `this` parameter directly.
   That naturally restores retail's `sw s7 / move s7,a0` prologue position.  The
   remaining three-line residual is one redundant `%lo(tournamentManager)` copy
...

{
  ...
  fadeline = fade;
  /* REGIONAL (all 6 retail regionals): +1 text-id shift on every string id in
   * this function (AUDIT_LO16: 39 words, all `+1` on a `li`/`addiu` immediate).
   * Per-id source-occurrence counts match the per-insn counts exactly (0x2f6 4,
   * 0x2fa 4, 0x2fb 5, 0x2fc 4, 0x2fd 2, 0x312/0x313/0x314/0x315 3 each), so no
   * constant is CSE-shared between two semantic classes (32B-5 checked).
   * NOT shifted (unchanged in the regional image, left alone): the `i += 0x13`
   * tournament-name offset, shape ids 0x27/0x28/0x1d, the RGB/geometry
   * literals and AudioCmn_PlayFESFX(0x15). */
  line = 0x2ff;
  ...
  for (; i < numRacers; i++) {
    ...
    FETextRender_FullTextFade(fade,TextSys_Word(i + 600),(short)TextSys_WordX(0x2f8),
    ...
    FETextRender_FullTextFade(
        ...
        (short)TextSys_WordX(0x2f9),(short)TextSys_WordY(line),
        type,state,0);
    p = j;
    if (tourneyInfo->fKnockout != 0) {
      sprintf(sBuildOutput,TextSys_Word(i == lastRacer ? 0x31d : 0x31c));
    }
    else {
      sprintf(sBuildOutput,"%d %s",(int)tournamentManager.TournPointTotal(&p),TextSys_Word(0x31e));
    }
    FETextRender_FullTextFade(fade,sBuildOutput,(short)TextSys_WordX(0x2fc),
                             (short)TextSys_WordY(line),type,state,1);
    line++;
  }
  trackManager.GetTrack((short)Front_GetTrackRaced(),trackInfo);
  FETextRender_FullTextFade(fade,TextSys_Word((short)Front_GetTrackRaced() + 0xd6),(short)TextSys_WordX(0x2f7),
                           (short)TextSys_WordY(0x2fe),type,textState_Hilighted,2);
  ...
  i += 0x13;
  FETextRender_MenuTextPositionedJustifyFade(fade,(short)i,(short)TextSys_WordX(0x2f7),(short)TextSys_WordY(0x2fd),
                                             2,textState_Hilighted,type);
  wwwww = textpixels(TextSys_Word(i));
  PSXDrawSquare(0,TextSys_WordX(0x2f7) - (wwwww >> 1),TextSys_WordY(0x2fd) - 1,wwwww,9);
  ...
  DrawShapeExtended(0x28,0,lbx + tt,TextSys_WordY(0x2fd) + 1,this->fScreenFadeVal,1,(tDrawShapeExtended *)0x0);
  DrawShapeExtended(0x28,0,lbx - tt,TextSys_WordY(0x2fd) + 1,this->fScreenFadeVal,1,(tDrawShapeExtended *)0x0);
  drawflags.tint[0] = 0x282828;
  DrawShapeExtended(0x27,0x400,0,-1,this->fScreenFadeVal,0,&drawflags);
  PSXDrawBrightEndLine(0x232323,TextSys_WordX(0x2f7) - 0x96,TextSys_WordY(0x2fe) + 10,
  ...
  if ((1000 < ticks[0] - this->starttick) || (this->fStartCountdownNOW != 0)) {
    ...
    this->moneyAwarded -= this->fCountSpeed;
  ...
  if (this->fDrawMoney != 0) {
    FETextRender_FullTextFade(fade,TextSys_Word(0x313),(short)TextSys_WordX(0x2fb),
                             (short)TextSys_WordY(0x313),
                             type,
                             this->gotmoney ? 1 : 0,1);
    DrawMoney(TextSys_WordX(0x2fc),TextSys_WordY(0x313),6,this->moneyAwarded,colf,colb);
    FETextRender_FullTextFade(fade,TextSys_Word(0x314),(short)TextSys_WordX(0x2fb),
                             (short)TextSys_WordY(0x314),
                             type,this->gotbilled ? 1 : 0,1);
    DrawMoney(TextSys_WordX(0x2fc),TextSys_WordY(0x314),6,this->moneyDamage,colf,colb);
    FETextRender_FullTextFade(fade,TextSys_Word(0x315),(short)TextSys_WordX(0x2fb),
                             (short)TextSys_WordY(0x315),
                             type,this->gotbonus ? 1 : 0,1);
    DrawMoney(TextSys_WordX(0x2fc),TextSys_WordY(0x315),6,this->moneyBonus,colf,colb);
  }
  FETextRender_FullTextFade(fade,TextSys_Word(0x316),(short)TextSys_WordX(0x2fb),(short)TextSys_WordY(0x316),
                           type,textState_Hilighted,1);
  DrawMoney(TextSys_WordX(0x2fc),TextSys_WordY(0x316),9,
...

{
  ...
  for (i = 0; state = textState_Hilighted,
      ...
      /* REGIONAL (all 6 retail regionals): +1 text-id shift on EVERY string id
       * in this function -- the retail table gained one entry ahead of them.
       * 0xd5->0xd6, 0x2f7->0x2f8, 0x2fe->0x2ff, 0x30d->0x30e, 0x2fb->0x2fc.
       * (The two `0x2fe + i` sites share one giv base `li $s1,0x2FE`; the two
       * `0x2fb` sites are separate `li`s -- both spellings shift together
       * because the whole table shifted, so the 32B-5 CSE trap does not bite
       * here.  Shape ids 0x27/0x28/0x1d and the geometry constants are NOT
       * text ids and are unchanged in the regional image.) */
      FETextRender_MenuTextPositionedJustifyFade((int)this->fScreenFadeVal,
                 (short)((signed char)trackInfo.fTrackID + 0xd6),
                 (short)TextSys_WordX(0x2f8),(short)TextSys_WordY(0x2ff + i),0,state,
                 type);
      if (frontEnd.pinkSlipsWinner[i] == -1) {
        sprintf(string,TextSys_Word(0x30e));
      }
      else {
        sprintf(string,PlayerName((int)frontEnd.pinkSlipsWinner[i]));
      }
      FETextRender_FullTextFade(fade,string,(short)TextSys_WordX(0x2fc),
                 (short)TextSys_WordY(0x2ff + i),type,state,1);
  }
  i = 0;
  ...
  do {
    /* REGIONAL: +1 text-id shift (continued) -- 0x2f8->0x2f9, 0x313->0x314,
     * 799->800, 0x31e->0x31f, 0x2fb->0x2fc, 0x2c1->0x2c2, 0x2f6->0x2f7,
     * 0x2fc->0x2fd. */
    FETextRender_FullTextFade(fade,PlayerName(i),(short)TextSys_WordX(0x2f9),
               (short)TextSys_WordY(0x314 + i),type,state,0);
    if (frontEnd.pinkSlipsWins[i] == 1) {
      sprintf(sBuildOutput,TextSys_Word(800));
    }
    else {
      sprintf(sBuildOutput,TextSys_Word(0x31f),(uint)(byte)frontEnd.pinkSlipsWins[i]);
    }
    FETextRender_FullTextFade(fade,sBuildOutput,(short)TextSys_WordX(0x2fc),
               (short)TextSys_WordY(0x314 + i),type,
               state,1);
    i = i + 1;
  } while (i < 2);
  FETextRender_MenuTextPositionedJustifyFade(fade,0x2c2,(short)TextSys_WordX(0x2f7),
               (short)TextSys_WordY(0x2fd),2,textState_Hilighted,type);
  wwwww = textpixels(TextSys_Word(0x2c2));
  PSXDrawSquare(0,TextSys_WordX(0x2f7) - (wwwww >> 1),
               TextSys_WordY(0x2fd) - 1,wwwww,9);
  ...
  tDrawShapeExtended drawflags;
  drawflags.tint[0] = 0x282828;
  DrawShapeExtended(0x28,0,lbx + tt,TextSys_WordY(0x2fd) + 1,
