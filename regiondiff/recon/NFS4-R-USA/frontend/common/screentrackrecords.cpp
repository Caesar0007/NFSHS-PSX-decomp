...

{
  ...
  if (-1 < this->TrackRecords[index].nBestLap) {
    if (newrecord != 0) {
      Flare_2DHalo(TextSys_WordX(0x248) + 3,y + 4,
                   this->flare_intensity / 2,
                   (this->flare_intensity * 2) / 3,0x17);
      DrawShapeExtended(0,0,TextSys_WordX(0x248),y,0,0,
                        (tDrawShapeExtended *)0x0);
    }
    FETextRender_FullText
              (this->TrackRecords[index].sName,(short)TextSys_WordX(0x24c),
               (short)y,textType_TrackRecords,
               newrecord != 0 ? textState_Hilighted : textState_Selected,0);
    FETextRender_FullText
              (TextSys_Word(this->TrackRecords[index].nCar + 0x154),
               (short)TextSys_WordX(0x24d),(short)y,
...

{
  ...
  if (8 < maxitem) {
    this->DrawOneRecord(0,NewBestLap,TextSys_WordY(0x261));
  }
  return;
}
