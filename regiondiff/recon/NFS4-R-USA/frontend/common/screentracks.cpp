...

{
  ...
  ::Initialize(&this->fVideoWall,this->tvConfigs,this->fSwapShapes.fShapes,
               0,10,tvOrder,0x96);
  /* REGIONAL DELTA (NFS4-R-USA @80041F4C): the video-wall's available-text word
     index is one higher than the base build's (0xf9 vs 0xf8) -- the retail
     string-table insertion.  Sole word-level delta in this function; the icon
     call's indices sit below the insertion point and are unchanged. */
