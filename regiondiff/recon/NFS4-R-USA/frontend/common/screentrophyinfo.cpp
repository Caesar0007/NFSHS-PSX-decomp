...
/* [NFS4-R-USA REGIONAL DELTA -- W86-B7] tScreenTrophyInfo::DrawBackground only.
   Retail's string table is shifted by one, so every TextSys word id this
   function materialises is +1 (8 words, AUDIT_LO16 / gate diff, count-exact
   298/298 before and after):
     title base   0x341 -> 0x342      heading id   0x37a -> 0x37b
     label ids    0x3db -> 0x3dc      body id      0x3a0 -> 0x3a1
                  0x3dd -> 0x3de                   0x38d -> 0x38e
                  0x3dc -> 0x3dd      footer id    0x367 -> 0x368
   NOTE the 0x3db/0x3dc/0x3dd chain: the ids form a run, so the three label
   constants MUST be rewritten SIMULTANEOUSLY -- a sequential edit would
   double-shift 0x3dc.  tScreenTrophyInfo::GetShapeInfo and the dtor are the
   base bodies, unchanged. */
...

{
  ...
  yyy = 0xaf;
  FETextRender_FullTextRGB(TextSys_Word((signed char)tourn->fTournamentID + 0x342),0x1e,0x19,col,'\x03',3);
  if (strlen(TextSys_Word(tournID + 0x37b)) != 0) {
    ...
    FETextRender_MenuTextPositionedJustifyFade(FadePartI,0x3dc,0x8c,0xaf,1,textState_Hilighted,textType_ScreenInfo);
    word = TextSys_Word(tournID + 0x37b);
    col = CalcFadeVal(0x505050,FadePartI);
    FETextRender_FullTextRGB(word,0x91,0xaf,col,'\0',0);
    yyy = 0xb7;
  }
  if (strlen(TextSys_Word(tournID + 0x3a1)) != 0) {
    FETextRender_MenuTextPositionedJustifyFade(FadePartI,0x3de,0x8c,yyy,1,textState_Hilighted,textType_ScreenInfo);
    r.x = 0x91;
    r.w = 0x15b;
    r.y = yyy;
    r.h = 100;
    FETextRender_WordWrapTextRGB(TextSys_Word(tournID + 0x3a1),r,
    ...
    yyy = yyy + FETextRender_WordWrapHeight(0x15b,
        TextSys_Word(tournID + 0x3a1));
  }
  if (strlen(TextSys_Word(tournID + 0x38e)) != 0) {
    FETextRender_MenuTextPositionedJustifyFade(FadePartI,0x3dd,0x8c,yyy,1,textState_Hilighted,textType_ScreenInfo);
    r.x = 0x91;
    r.w = 0x15b;
    r.y = yyy;
    r.h = 100;
    FETextRender_WordWrapTextRGB(TextSys_Word(tournID + 0x38e),r,
        CalcFadeVal(0x505050,FadePartI));
    FETextRender_WordWrapHeight(0x15b,TextSys_Word(tournID + 0x38e));
  }
  ...
  r.w = 0x1c4;
  r.h = 100;
  FETextRender_WordWrapTextRGBJustify(TextSys_Word(tournID + 0x368),r,
