...

{
  ...
  if (0 < menu_kUserNameRows) {
    do {
       ...
       * keyboard-row word base (0x1fb -> 0x1fc; AUDIT_LO16 insn 24, an
       * `addiu $a0,$s0,508` — the loop index is the running $s0). */
  ...
  while (row < menu_kUserNameRows) {
    ...
    while (this->fRowList[0][col + row * 9] != '\0') {
      ...
      case '#':
        ...
        break;
      case '$':
        FETextRender_FullTextRGB(TextSys_Word(0x207),x + 0x24,y - 1,colText,1,2);
        for (k = 1; k < 3; k++) {
          PSXDrawSquare(0,x + k * 0x1c - 6,y - 2,2,0xe);
        }
        break;
      case '&':
        FETextRender_FullTextRGB(TextSys_Word(0x208),x + 0x16,y - 1,colText,1,2);
        PSXDrawSquare(0,x + 0x16,y - 2,2,0xe);
        break;
      case '^':
        FETextRender_FullTextRGB(TextSys_Word(0x208),x + 0x4e,y - 1,colText,1,2);
        for (k = 1; k < 6; k++) {
          PSXDrawSquare(0,x + k * 0x1c - 6,y - 2,2,0xe);
        }
        break;
