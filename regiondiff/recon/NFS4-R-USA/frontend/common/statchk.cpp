...

{
  ...
  bTopTenFlag = false;
  nPlace = 0;
  /* REGIONAL DELTA: retail emits the s6 zero BEFORE the record-size li 20,
     i.e. the size assignment follows the flag inits (still in the same
     basic block, above the topPlacements loop, so W71's cse-opacity holds). */
  ...
  /* REGIONAL DELTA (retail): the base build's placement-collision fixup
     (topPlacements[0]==topPlacements[1] adjust + the <9 guarded write) is
     REPLACED by a rescan of the eight top-ten slots per player: the slot
     whose time AND name match that player is the one flagged new, and its
     time is zeroed so the second player cannot re-claim it. */
  k = 0;
  while (1) {
    if (k >= 2) break;
    if (topPlacements[k] != 0) {
      for (nRec = 1; nRec < 9; nRec = nRec + 1) {
        if ((RecordHolders[nLapIndicator + nRec - 1].nTime ==
             dummyCars[k].finalTotalTime) &&
            (strcmp(RecordHolders[nLapIndicator + nRec - 1].sName,
                    PlayerName((int)k)) == 0)) {
          NewRecords[nRec - 1] = 1;
          RecordHolders[nLapIndicator + nRec - 1].nTime = 0;
        }
      }
    }
    k = k + 1;
