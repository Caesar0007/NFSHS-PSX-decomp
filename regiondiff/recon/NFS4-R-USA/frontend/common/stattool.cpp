...

{
  ...
  for (i = 1; i < nNumber; i++) {
    nADummy = nTemp[i];
    nBDummy = nIndex[i];
    /* REGIONAL DELTA (NFS4-R-USA @8004AD40): retail shifts on EQUAL keys too --
       oracle `slt $v0,$v1,$t0 / bnez` = exit only when nTemp[j] < nADummy, i.e.
       the loop condition is `nADummy <= nTemp[j]` (the base build's strict `<`
       emits the inverted `slt $v0,$t0,$v1 / beqz`).  Behavioural retail change:
       the insertion sort stops being stable. */
...

{
  ...
  case 0xE8:
    ...
    /* REGIONAL DELTA (NFS4-R-USA @8004B064): retail HAS the break here -- the
       0xE8 arm returns instead of falling through into 0xEC (the base build
       falls through, ending the arm on a bare `sb` with no `jr ra`, one insn
       short).  gcc then cross-jumps the else-path's `li 69 / jr ra / sb` onto
       case 0xE9's shared tail (.L8004B008), which is why the oracle's `bne`
       carries `li $v0,69` in its delay slot. */
    return;
  case 0xEC:
    *c = 0xCC;
    return;
