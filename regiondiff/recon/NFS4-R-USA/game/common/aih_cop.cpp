...

{
  ...
  case 4:
    ...
    /* REGIONAL DELTA (retail, all 6 regionals; base NFS4-B-USA lacks it):
       a null-perp guard added at the head of the chase arm.  Retail bails out
       of the whole chase body straight into the drive-away tail when the cop
       has no perp assigned.  Regional oracle @80064C48 (base has nothing here):
         lw $v0,88($s1) ; nop ; beqz $v0,.L80065208 ; li $a2,1
       (the `li $a2,1` is reorg filling the slot from the target block's
       `direction = 1`, exactly as at the two base-tree goto sites). */
