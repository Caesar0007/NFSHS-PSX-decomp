 ...
 * AudioClc_SilenceOpponentHorn and AudioClc_SoundCloseCar (61 insns).
 * Keeps the mechanical func_<region_va> name (no ground-truth name exists). */
extern "C" int func_8007593C(void);
...
/* ---- func_8007593C  [REGION-ONLY @0x8007593C, NFS4-R-USA] ------------------
...
void AudioClc_SoundCars(void)
{
  ...
  if (GameSetup_gData.commMode == 1) {
    ...
    RDAT_8013D8F8 = func_8007593C();   /* REGIONAL */
    AudioClc_SoundPlayersCar(0);
    if (0 < gMasterAmbientLevel) {
      AudioClc_SoundCloseCar(0, 0);
      AudioClc_SoundCloseCar(0, 1);
