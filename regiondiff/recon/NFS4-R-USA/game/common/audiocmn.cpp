...
int AudioCmn_PlayDoppleredSound(int bhandle,int patchNum,int azimuth,int vol,int bend,int doppler)
{
  ...
  if (patchNum == 0x7d) {
    ...
    playopts.vol = (u_char)(gMasterEngineLevel * (((vol * 3) << 3) + vol) >> 0xe);
  }
  else {
  ...
  if (Audio_direct3davail != 0) {
    if (azimuth <= 0xbfff) {
      if (azimuth >= 0x4000) {
        if (azimuth > 0xa000) {
          azimuthScale = 0xc000;
          azimuthScale -= azimuth;
          goto APPLY_AZIMUTH_SCALE;
        }
        if (!(azimuth < 0x5fff)) goto DOUBLE_AZIMUTH_AMP;
        azimuthScale = azimuth;
        azimuthScale -= 0x3fff;
APPLY_AZIMUTH_SCALE:
        iAmpIn += (iAmpIn * azimuthScale) >> 0xd;
        goto AZIMUTH_ADJUSTED;
DOUBLE_AZIMUTH_AMP:
        iAmpIn <<= 1;
      }
    }
...
GOTBANK:
  if (sndPlayer == 0x31) {
    ...
    /* Retail computes this flag before the PatchBank < -1 guard. */
...
RECHECK:
    ...
    if (iSFXnum == 0x7d) {
      ...
      SNDvol(gaChannel[sndPlayer].Partial,
             gMasterEngineLevel * (((iAmp * 3) << 3) + iAmp) >> 0xe);
    }
    else {
      SNDvol(gaChannel[sndPlayer].Partial,
...
SoundCar_haveWetNoise:
  ...
  roadNoiseAmp = roadNoiseAmp * *(int *)0x8013d8f8 >> 7;
  wetNoiseAmp = wetNoiseAmp * *(int *)0x8013d8f8 >> 7;
  if (roadNoiseAmp != 0) {
    sndPlayer = 0x19;
    if (car->carIndex == 0) {
      sndPlayer = 0x18;
