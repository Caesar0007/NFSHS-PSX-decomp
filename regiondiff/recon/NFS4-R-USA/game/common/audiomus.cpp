...
/* REGIONAL DELTA (retail): AudioMus_tMusicGlobals LOSES the `failby` field
   (base +0x1C), so every member from `errorcode` on moves down 4 bytes.
   Oracle evidence: errorcode 0x20->0x1C, fadetime 0x24->0x20, songname
   0x2C->0x28, streamhandle 0x74->0x70, streamstatus 0x90->0x8C,
   current 0x118->0x114.  (The same shift is visible in the regional
   AudioMus_RefreshStatus @8007A960 -- `lw a0,112(a1)` / `addiu a1,a1,140` --
   even though the corpus does not list that function as CHANGED.)
   The base tree's shared header must not be edited, so the regional layout is
   declared here and the type name is re-pointed for the rest of this TU. */
struct AudioMus_tMusicGlobalsRegional {
    int volume, totalsongs, availablesongs, requestsong, firstswitch, switchsong;
    int newswitch, errorcode, fadetime, randomize;
    char *songname;
    char title64[64];
    char *streambuffer;
...
#define AudioMus_tMusicGlobals AudioMus_tMusicGlobalsRegional
...
void AudioMus_Fail(int errorcode)
{
  ...
  /* REGIONAL DELTA: retail short-circuits the -5 (out-of-data) failure --
     it only arms switchsong and leaves the rest of the state alone. */
  if (errorcode == -5) {
...
void AudioMus_QueueRequestedSong(void)
{
  ...
  AudioMus_g->switchsong = 2;
  /* REGIONAL DELTA: `failby = gettick() + 0x280;` is gone with the field
     (no jal in the regional oracle between the switchsong store and the
     current-song clear). */
  pAVar2 = AudioMus_g;
...
int AudioMus_Server(int mode,int ticks)
{
  ...
  /* REGIONAL DELTA: the whole CD-swap preamble is gone -- retail drops the
     `bigfilename[0] == '.'` / CdDiskReady(1)==0x10 disc-present arm AND the
     errorcode==-2 "wait for disc" arm; the server goes straight to
     RefreshStatus once a big-file header exists. */
...
switchsong_default:
  ...
  /* REGIONAL DELTA: `failby = gettick() + 0x280;` removed with the field. */
  goto done;
