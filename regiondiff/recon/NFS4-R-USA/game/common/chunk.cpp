...
/* [INFERRED] REGION-LAYOUT GLOBAL: an int counter at NFS4-R-USA VA 0x8013E7B8,
 * incremented once per live type-5 instance by the regional census block below.
 * Identity TBD -- a regional VA must never be resolved through the base layout's
 * configs/symbol_addrs.txt, and no caller/string in this region's INDEX.tsv names
 * it, so it keeps the mechanical RDAT_<reprRegionVA> name (regiondiff naming rule).
 * Absolute (lui/lw + lui/sw) in the oracle, i.e. NOT gp-relative / not TU-owned. */
extern int RDAT_8013E7B8;
...
void Chunk::InstanceGroup(SerializedGroup *chunkGroup, SimpleMem *mem)
{
  ...
  if (CHUNK_COMMMODE != 1) {
    ...
    if (simGroup == (SerializedGroup *)0x0) {
      ...
      pGVar6 = (chunkGroup)->CreateLiteGroupDataSize(simGroup, mem, numElements * 0x14);
      this->simObjBuf = pGVar6;
...
InstanceGroup_instancesDone:
  ...
  /* ---- REGIONAL DELTA (NFS4-R-USA retail @8008C1BC): a census pass the matched
   * base build does not have -- walk the instance buffer and bump a global counter
   * once per live type-5 (sim-object) instance.  Reached from BOTH arms of the
   * CHUNK_COMMMODE test (the non-COMMMODE arm's `j .L8008C1BC` lands here). ---- */
  if (this->objInstanceBuf != (Group *)0x0) {
    Trk_SimpleInst *inst;
    int numElements;
    int i;
