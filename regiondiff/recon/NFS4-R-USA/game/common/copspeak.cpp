...
int                RDAT_8013E3B8; /* [INFERRED] retail-only gp+0xB7C cell; identity TBD */
...

{
  ...
  if ((int)CopSpeak_gBufferLow < (int)CopSpeak_gBufferStart) {
  ...
  if ((-1 < iVar1) && (iVar2 = r->size, 0 < iVar2)) {
    if (CopSpeak_gBufferHigh != 0) {
      if (iVar1 + iVar2 != (int)CopSpeak_gBufferHigh) {
        CopSpeak_gBufferLow =
            (u_short)CopSpeak_gBufferLow + (u_short)r->size;
        CopSpeak_gBufferEnd = CopSpeak_gBufferLow;
      }
      else {
...

{
  ...
  if (r->buffer >= 0) {
    ...
    if (r->ophandle != -1) {
      opstatus = FILE_opstatus(r->ophandle);
      if (opstatus == 0) {
        return;
      }
      ok = opstatus == 1;
      completed = FILE_completeop(r->ophandle);
      r->ophandle = -1;
    ...
    if (!ok) {
      ...
      if (*(signed char *)&r->bank >= 0) {
        if ((r->sfx != 0) || (r->phrase >= 0)) {
          if (AudioCmn_GetAsyncSfx(*(signed char *)&r->bank,r->phrase,false) == -1) {
          ...
          handle = AudioCmn_GetAsyncSfx(*(signed char *)&r->bank,
                                        r->offset + 0x4000,false);
          if (handle == -1) {
            AudioCmn_LoadAsyncSfx(*(signed char *)&r->bank,r->offset + 0x4000,
                                  CopSpeak_gBuffer + r->buffer,r->size);
            if (AudioCmn_GetAsyncSfx(*(signed char *)&r->bank,
                                     r->offset + 0x4000,true) == handle) {
              RDAT_8013E3B8 = 1;
              ok = false;
            }
          }
...

{
  ...
  next = CopSpeak_gQueueLoad - 0x40;
  if (((CopSpeak_gQueueHead - next) & 0x3f) < 0x21) {
...

{
  ...
  queueBase = CopSpeak_gQueueLoad - 0x40;
  if (0x20 < ((head - queueBase) & 0x3f)) {
    return -1;
  }
  iVar6 = 0;
  if (head < 0x3f) {
...

{
  ...
  while (true) {
    ...
    queueBase = CopSpeak_gQueueReady - 0x40;
    if (7 < ((CopSpeak_gQueueLoad - queueBase) & 0x3f)) {
      return;
    }
    r = &CopSpeak_gQueue[CopSpeak_gQueueLoad];
    if (*(signed char *)&r->bank < 0) {
    ...
    if ((CopSpeak_gBuffer == (char *)0x0) || (CopSpeak_BankVolume(r) == 0) ||
        ...
        ((Replay_ReplayMode >= 2) && (Replay_ReplayInterface.speed == 3))) {
      CopSpeak_Skip();
      continue;
    }
    CopSpeak_Alloc(r);
