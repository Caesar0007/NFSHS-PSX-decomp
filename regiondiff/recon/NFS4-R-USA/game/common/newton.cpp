  ...
  for (i = 0;
    ...
    if (((roadNormal.y < 0x1999) || (roadSurfaceType == 0xe)) || (roadSurfaceType == 0)) {
      ...
      if (testSimRoadInfo.simQuad != (Trk_NewSimQuad *)0x0) {
        ...
        roadCenterPoint = *(coorddef *)BWorldSm_slices[testSimRoadInfo.slice].center;
      }
...

{
  ...
  newHeight = 0;
  /* REGIONAL DELTA (NFS4-R-USA @800A39D0): retail clears the NORMAL as well, before
     the road-centre copy (oracle: two `jal func_800E4F04` with `li $a2,12`, the first
     on `addiu $a0,$sp,16` = &normal, the second on `$sp,64` = &speedVec). */
  memset((u_char *)&normal,'\0',0xc);
  ...
  do {
    ...
    if ((signed char)testSimRoadInfo.offEdge != 0) {
      ...
      if (Netwon_CheckForBadQuad(newtonObj,&testSimRoadInfo,i) != 0) {
        ...
        /* REGIONAL DELTA (NFS4-R-USA @800A3C54): retail carries one more arm on this
           chain -- when FindEdgeOff reported NO edge (0) or BOTH pairs (12) the
           surface normal is taken as the negated linear velocity instead of a
           generated edge vector.  Oracle: `beqz $s0,.L800A3C64; li $v0,12; bne
           $s0,$v0,.L800A3C98` then three `lw 172/176/180($s1); negu; sw 16/20/24($sp)`
           into &normal.  This block IS the whole 22-instruction gap (470 -> 492). */
        else if ((check == 0) || (check == 12)) {
          normal.x = -(newtonObj->linearVel).x;
          normal.y = -(newtonObj->linearVel).y;
          normal.z = -(newtonObj->linearVel).z;
        }
        if ((check & 3) != 0) {
          if ((check & 1) != 0) {
            Newton_GenerateVector(
