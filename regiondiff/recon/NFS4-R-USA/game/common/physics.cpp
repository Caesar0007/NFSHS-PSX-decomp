...

{
  ...
  /* REGIONAL DELTA (NFS4-R-USA retail): the retail build tests the SGGE cheat BIT
   * (`andi 0x80; beqz`) where the matched base build compares the whole flag word
   * (`li 128; bne`) -- same word GameSetup_gData[14], bit test instead of equality. */
  if ((((PHYSICS_SGGE & 0x80U) != 0) &&
