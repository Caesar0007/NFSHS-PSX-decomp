...
/* USA retail selects the per-view budget path through this regional flag. */
extern int Draw_gUseViewMemBudget;
...
{
  Draw_gView[viewid].membudget = totalmem - 0x1a0;
  return;
}
...

{
  ...
  if (Draw_gUseViewMemBudget != 0) {
    sd->head.cprim.MPrimPtr = sd->head.cprim.PrimPtr + view->membudget;
  }
  else {
    char *maxPrim = Draw_gMaxPrim - 0x1a0;
    if ((Draw_GameSetupWords[3] == 1) && (viewid == Draw_gPlayer2View)) {
      char *prim = sd->head.cprim.PrimPtr;
      sd->head.cprim.MPrimPtr = prim + ((maxPrim - prim) >> 1);
    }
    else {
      sd->head.cprim.MPrimPtr = maxPrim;
    }
  }
  sd->head.mirror = 0;
  return;
