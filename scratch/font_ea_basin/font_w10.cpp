...
struct PSXFontHdr { u_char flags; u_char pad[11]; int vramLoc; };
...

{
  ...
  width = ch->width;
  height = ch->height;
  dv = ((((PSXFontHdr *)src)->vramLoc << 4) >> 0x14) + v & 0xff;
  ...
  *(u_long *)&prim->r0 = font_tint;
  ...
  dv += height,
  prim->u2 = u, prim->v2 = dv,
  prim->u3 = u + width, prim->v3 = dv;
  height = y + height, width = x + width,
  prim->x0 = x, prim->y0 = height,
  prim->x1 = width, prim->y1 = height,
  prim->x2 = x, prim->y2 = y,
  prim->x3 = width, prim->y3 = y;
  return;
}
