...
bld.OUT = bld.BUILD
# --- w47-a6 flag probe: inject extra PER_TU_FLAGS keys from env NFS4_A6_FLAGS
# (comma-separated key names, e.g. "no_delayed_branch,no_schedule_insns").
# Leaves tools/build.py untouched; probe-only.
sys.path.insert(0, str(ROOT / 'tools'))
_a6_extra = [k for k in os.environ.get('NFS4_A6_FLAGS','').split(',') if k]
if _a6_extra:
    _a6_orig = bld.per_tu_flags
    def _a6_patched(src, _o=_a6_orig, _e=_a6_extra):
        d = dict(_o(src))
        for k in _e:
            if '=' in k:
                kk, vv = k.split('=', 1); d[kk] = vv
            else:
                d[k] = True
        return d
    bld.per_tu_flags = _a6_patched
    sys.stderr.write('[a6] extra per-TU flags: ' + ','.join(_a6_extra) + os.linesep)
    # force a distinct output dir so probe objects never clobber the real gate's
    bld.OUT = bld.BUILD / ('a6probe_' + '_'.join(re.sub(r'[^A-Za-z0-9]', '', x) for x in _a6_extra))

# --- w47-a6 ASPSX `.set reorder` emulation between cc1 and maspsx (probe) ---
if os.environ.get('NFS4_A6_ASPSX') == '1':
    sys.path.insert(0, str(ROOT / 'scratch'))
    import w47_a6_aspsxreorder as _a6r
    _splice_orig = bld._apply_fn_splice
    def _a6_splice(rel, s_file, i_file, cc, flags, _o=_splice_orig):
        _o(rel, s_file, i_file, cc, flags)
        p = str(s_file)
        with open(p, 'r', encoding='utf-8', newline='') as _fh:
            _src = _fh.read()
        with open(p, 'w', encoding='utf-8', newline='') as _fh:
            _fh.write(_a6r.reorder_text(_src))
    bld._apply_fn_splice = _a6_splice
    bld.OUT = bld.BUILD / ('a6probe_aspsx_' + '_'.join(
        re.sub(r'[^A-Za-z0-9]', '', x) for x in _a6_extra))
