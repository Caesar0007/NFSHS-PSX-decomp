...
sys.path.insert(0, str(ROOT / 'tools'))   # w47-a9: this copy lives in scratch/
...
# --- w47-a9 FLAG-PROBE HOOKS (env-driven; tools/build.py is NOT edited) -----
#   NFS4_A9_GVALUE     -> override -G for cc1 AND maspsx AND the C++ per-TU key
#   NFS4_A9_CC1FLAGS   -> extra cc1/cc1plus flags (shell-split), appended last
#   NFS4_A9_CC1        -> swap the C-lane compiler binary (near-oracle probes)
# Everything else -- maspsx, GNU as, the oracle diff, the normalizer -- is the
# stock gate, so a number produced here is comparable with tools/verify_asm.py.
import shlex as _shlex
_g = os.environ.get('NFS4_A9_GVALUE')
if _g:
    bld.G_VALUE = _g
    bld.CC1_FLAGS = [f for f in bld.CC1_FLAGS if not f.startswith('-G')] + ['-G' + _g]
    _rel = cpp.relative_to(ROOT).as_posix()
    bld.PER_TU_FLAGS[_rel] = dict(bld.PER_TU_FLAGS.get(_rel, {}), g_value=_g)
_extra = _shlex.split(os.environ.get('NFS4_A9_CC1FLAGS', ''))
if _extra:
    bld.CC1_FLAGS = list(bld.CC1_FLAGS) + _extra          # C lane (+ fn-splice)
    _orig_run = bld.run
    def _run_a9(cmd, **kw):                                # C++ lane
        cmd = list(cmd)
        if str(cmd[0]) == str(bld.CC1PL):
            cmd = [cmd[0]] + _extra + cmd[1:]
        return _orig_run(cmd, **kw)
    bld.run = _run_a9
_cc = os.environ.get('NFS4_A9_CC1')
if _cc:
