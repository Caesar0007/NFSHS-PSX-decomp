...
_ps = _os.environ.get('W60_FN_SPLICE_FILE')
if _ps:
    _spec = _json.loads(open(_ps,'rb').read().decode())
    _orig_splice = bld._apply_fn_splice
    def _w60_splice(rel_posix, s_file, i_file, cc1_bin, cc1_flags):
        _orig_splice(rel_posix, s_file, i_file, cc1_bin, cc1_flags)
        tbl = _spec.get(rel_posix)
        if not tbl:
            return
        for _i, (flag, names) in enumerate(sorted(tbl.items())):
            ff = list(cc1_flags)
            if flag not in ff:
                ff.append(flag)
            fs = s_file.with_suffix('.w60s%d.s' % _i)
            r = bld.run([cc1_bin, *ff, i_file, '-o', fs])
            if r.returncode:
                raise SystemExit('[w60-splice] ' + r.stdout + r.stderr)
            ft = fs.read_text(); nt = s_file.read_text()
            for nm in sorted(names):
                fr = bld._extract_fn_region(ft, nm)
                tr = bld._extract_fn_region(nt, nm)
                bld._SPLICE_COUNTER[0] += 1
                fr = bld._uniquify_local_labels(fr, 'w60s%d' % bld._SPLICE_COUNTER[0])
                nt = nt.replace(tr, fr, 1)
            s_file.write_text(nt)
    bld._apply_fn_splice = _w60_splice
