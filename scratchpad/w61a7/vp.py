...
ROOT = Path(r'C:/Temp/nfs4-decomp')   # tools/ -> repo root (sbsx parents[2] lesson)
...
_ns = _os.environ.get('W61_NOSPLIT')
if _ns:
    for _k,_v in _json.loads(_ns).items():
        bld.PER_FN_NO_SPLIT_ADDRESSES.setdefault(_k, set()).update(_v)
_vs = _os.environ.get('W61_VERSPLICE')
if _vs:
    for _k,_v in _json.loads(_vs).items():
        bld.PER_FN_CC1_VER_SPLICE_272[_k] = {_a:set(_b) for _a,_b in _v.items()}
