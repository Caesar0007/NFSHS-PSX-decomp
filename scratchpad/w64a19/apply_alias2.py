...
def scan_decls(data, pat):
    ...
    for idx, raw in enumerate(ls):
        ...
        s = L.strip()
        cand = None
        if (line_start_ok and m and "asm(" not in L
                and not incomment
...
def main():
    prev = json.loads((ROOT / "scratchpad/w64a19/apply_result.json").read_text())
    ...
    for src in sorted(bytu):
        ...
        compile_one(src)
    log.append(f"PASS2 FIXED {len(fixed)} / FAILED {len(failed)}")
    (ROOT / "scratchpad/w64a19/apply_log2.txt").write_text("\n".join(log) + "\n")
    json.dump({"fixed": fixed, "failed": failed},
              open(ROOT / "scratchpad/w64a19/apply_result2.json", "w"), indent=1)
    print("\n".join(log[-30:]))
