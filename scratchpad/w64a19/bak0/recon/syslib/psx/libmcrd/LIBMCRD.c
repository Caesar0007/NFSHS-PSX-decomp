                                   ...
                                   long ofs, long max)
{
    ...
    if (ofs + max > 0) {
        do {
            if (idx == 0) {
                /* W64-A4 (loop.c-cited, see the LICM receipt above the fn): the retry
                 * loop is a LABEL+goto loop, NOT a `while (1)`.  A structured inner loop
                 * plants NOTE_INSN_LOOP_BEG/END, so loop.c runs a whole extra pass over
                 * it and hoists the `mc` base out of it FIRST; move_movables then sets
                 * moved_once[base] and DOUBLES insn_count for the entire OUTER movable
                 * list (loop.c:1618-24, the state persists across the list).  That
                 * doubling (67 -> 134) is exactly what puts the movstrsi end-pointer
                 * `&ent+32` under the bar (threshold 24 * savings 1 * life 5 = 120 < 134)
                 * so it is recomputed inside the copy loop instead of being hoisted and
                 * spilled the way retail does.  A goto loop carries no loop notes =>
                 * loop.c never sees it => no first hoist => no doubling => 120 >= 67 and
                 * the end pointer moves to the outer preheader. */
retry_first:
                    _clr_card_event();
