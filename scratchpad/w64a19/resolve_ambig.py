...
def receiver(sym):
    ...
    if not m:
        return None, None
    base, ln, cls = m.group(1), int(m.group(2)), m.group(3)
    return base, cls[:ln]
...
def main():
    ...
    log.append(f"PASS4 FIXED {len(fixed)} / FAILED {len(failed)}")
    log.append("=== NOT AUTO-RESOLVABLE ===")
    for u in unresolved:
        log.append(f"  {u[0]}  ({u[1]})  objs={[x.split('build/')[-1] for x in u[2]]}"
                   f"  cands={u[3][:6]}")
    (ROOT / "scratchpad/w64a19/apply_log4.txt").write_text("\n".join(log) + "\n")
    json.dump({"fixed": fixed, "failed": failed, "unresolved": unresolved},
              open(ROOT / "scratchpad/w64a19/apply_result4.json", "w"), indent=1)
    print("\n".join(log[-40:]))
