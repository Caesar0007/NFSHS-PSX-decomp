"""w64a7 multi-anchor in-place variant probe.

Usage: python scratchpad/w64a7/probe2.py <relpath> <fn> <variants.py>
variants.py must define VARIANTS = [(name, [(anchor, replacement), ...]), ...]
Every anchor must occur exactly once.  Restores the TU in finally.
"""
...
def main():
    ...
    with open(os.path.join(HERE, 'orig2.bak'), 'wb') as f:
        f.write(orig)
    results = []
    try:
    ...
    with open(os.path.join(HERE, 'probe2_out.txt'), 'w') as f:
