#!/usr/bin/env python3
"""W66-A4  gen_ld.py -- generate `linkers/nfs4_recon.ld` and link it for real.

W66-A4 EXTENSION (the other half of LOCALIZE).  W65-A5 excluded the splat
blobs' `_jNN` jump-table pieces from the recon link because the recon TU emits
that table itself -- but it never PLACED the TU's own `.rodata`, so the table
landed in the trailing `.rodata_rest` catch-all and every
`%hi/%lo(.rodata+N)` switch dispatch in that TU's `.text` resolved to a
non-retail address.  This generator now also emits, per anchored TU, a
`.roNNNN <retail base> : { <obj>(.rodata); }` region, taken from
scratchpad/w66a4/rodata_placement.json (produced by rodownmap.py + mkplace.py:
jump-table-content anchor, unique-masked-ROM-match anchor, ownmap E1-E5).
It also places the SECOND rodata blob (`rdata_80054548`, residual `_rNN`
pieces) in a new `.rodata_main` region -- at HEAD those pieces had no address
at all.  The text spine has a clean hole over 0x80054548..0x8005797C (measured:
ZERO text objects intersect it; ai.cpp.o starts exactly at the blob end), so
the region cannot collide with the spine.

Original W65-A5 docstring follows.
...
# W66-A4: the second splat rodata blob had NO placed region at HEAD -- its
# residual `_rNN` pieces fell into `.rodata_rest` with no address.  The text
# spine leaves 0x80054548..0x8005797C completely free (asserted below).
...
main_ro = []
if mpieces:
    assert not mwhole.exists(),         "both the whole blob and its pieces exist -- refusing an ambiguous link"
    for p in mpieces:
        main_ro.append((piece_va(p), p.name,
                        re.search(r"_(j)\d\d\.", p.name) is not None))
    main_ro.sort()
    assert main_ro[0][0] == MAIN_RODATA_START,         f"first main-rodata piece is at {main_ro[0][0]:#x}, not the blob base"
else:
...
# the spine must not reach into the region (anti-vacuity for the new region)
for base, sz, o in placed:
    assert base >= MAIN_RODATA_END or base + sz <= MAIN_RODATA_START,         f"text object {o} at {base:#x}+{sz:#x} intersects the rodata-2 span"

# ------------------------------------------- the recon TUs' own .rodata windows
RO_SPANS = [(OVERLAY_START, 0x800128F0), (MAIN_RODATA_START, MAIN_RODATA_END)]
PLACEMENT = HERE / "rodata_placement.json"
...
if PLACEMENT.exists() and "--no-rodata" not in sys.argv:
    ...
    recon_ro.sort()
    # E2-style guard: the windows must not overlap EACH OTHER
    for a1, b1 in zip(recon_ro, recon_ro[1:]):
        assert a1[1] <= b1[0],             f"recon .rodata windows overlap: {a1[2]} {a1[0]:#x}..{a1[1]:#x} vs "             f"{b1[2]} {b1[0]:#x}..{b1[1]:#x}"
