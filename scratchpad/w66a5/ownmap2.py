...
def main():
    ...
    tu_defined = set()
    excluded = set()
    binding = defaultdict(set)          # name -> {(obj,sec) defining it GLOBALLY}
    localdef = defaultdict(set)         # name -> {(obj,sec) defining it LOCALLY}
    raw = defaultdict(list)
    for obj, d in data.items():
        for name, sec, off, glob in d["syms"]:
            if name in by_name and sec in DATA_SECS:
                raw[(obj, sec)].append((name, off, glob))
                (binding if glob else localdef)[name].add((obj, sec))

    # (D2) W66-A5 CORRECTION -- LOCAL BINDING DISAMBIGUATES.  Guard (D) below was
    # written when maspsx re-emitted every .bss/.sbss entry with an unconditional
    # `.globl`, which made the five file-static `gSwapFileName` copies globally
    # indistinguishable from screentrophyroom's one real global.  The maspsx
    # .lcomm guard (W63) fixed that at the source: those five are now `l .bss`
    # (objdump-verified this run).  A LOCAL definition of a name is a DIFFERENT
    # OBJECT that merely shares a spelling -- it can never be the owner of a blob
    # label that some other TU defines globally.  So: if exactly one TU defines
    # the name globally, that TU is the anchor and every local homonym is dropped
    # from the window membership entirely (it is not "our copy of the label").
    dropped_homonyms = []
    for (obj, sec), members in raw.items():
        for name, off, glob in members:
            if not glob and len(binding.get(name, ())) == 1:
                dropped_homonyms.append((obj, sec, name))
                continue
            wins[(obj, sec)].append((name, off))
            tu_defined.add(name)
    if dropped_homonyms:
        print(f"local homonyms dropped (a global owner exists): "
              f"{len(dropped_homonyms)} "
              f"{sorted({n for _, _, n in dropped_homonyms})[:6]}")
    for k in list(wins):
