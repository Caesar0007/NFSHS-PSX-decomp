        ...
        for sec, members in sorted(secs.items()):
            ...
            for i, (off, name) in enumerate(members):
                ...
                if len(ob) != n or len(rb) != n:
                    # W66-A5 ANTI-VACUITY FIX.  A blob RECORD runs to the next
                    # LABEL, so it routinely includes the object's trailing
                    # alignment pad -- and our section legitimately stops at the
                    # object's real size.  Declaring that UNKNOWN threw the whole
                    # symbol away and HID A REAL DEFECT (drawshp `kNoColor`:
                    # retail 80 80 80 00, ours 0 -- 4 readable bytes behind an
                    # 8-byte record).  Compare the OVERLAP instead; only a symbol
                    # with NO readable overlap is genuinely unknown.
                    m = min(len(ob), len(rb))
                    if m == 0:
                        unknown.append((obj, sec, name, "no readable overlap"))
                        continue
                    unknown.append((obj, sec, name,
                                    f"short read (compared {m} of {n} B)"))
                    rb, ob = rb[:m], ob[:m]
                checked += 1
                if set(rb) <= {0}:
