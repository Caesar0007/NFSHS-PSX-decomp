...
# W84-C1: this is a COPY of tools/verify_asm.py living in scratchpad/w84/, so the
# repo root is two levels further up than the original's parent.parent.
...
sys.path.insert(0, str(ROOT / 'tools'))   # W84-C1: build.py's `import fix_symsizes`
...
# ---- W84-C1 PROBE-ONLY extra cc1 flags (env C1_EXTRA_CC1_FLAGS, space separated)
# compile_c builds its flag list from the module-level CC1_FLAGS, so appending here
# reaches the C lane without touching tools/*.py.  PROBE ONLY -- never a seal lane.
_x = os.environ.get('C1_EXTRA_CC1_FLAGS', '').split()
if _x:
    bld.CC1_FLAGS = list(bld.CC1_FLAGS) + _x
# ---- W84-C1 PROBE-ONLY cc1 binary override (env C1_CC1_BIN): lets an UNTRIED
# RETAIL binary be measured without adding a rung row to tools/build.py.
_b = os.environ.get('C1_CC1_BIN')
if _b:
    bld.CC1 = _b
_d = os.environ.get('C1_DROP_CC1_FLAGS', '').split()
if _d:
    bld.CC1_FLAGS = [f for f in bld.CC1_FLAGS if f not in _d]
    for _n, _t in vars(bld).items():          # drop the TU-flag that re-adds it
        if isinstance(_t, dict):
            for _k, _v in list(_t.items()):
                if isinstance(_v, dict) and 'no_split_addresses' in _v and \
                        '-mno-split-addresses' in _d:
                    _v = dict(_v); _v.pop('no_split_addresses'); _t[_k] = _v

# ---- W84-C1 LANE ALIASING (pattern from regiondiff/tools/verify_region.py ~L118-130) ----
# The scratch copy lives outside recon/, so build.py's exact-rel-keyed lane tables
# (flag overrides, per-fn splices, compiler-version rows) would NOT apply and the
# gate would measure a DIFFERENT toolchain than the TU's real one.  Fix: copy the
# candidate BESIDE the base TU (sibling includes + dir-prefix class rules resolve
# identically) and alias every module-level dict entry keyed by the base rel path
# onto the temp rel path.
LANE_AS = os.environ.get('C1_LANE_AS', 'recon/syslib/psx/libgpu/FONT.c')
_tmp = None
try:
    _src = cpp
    _base = ROOT / LANE_AS
    if _base.exists() and cpp.resolve() != _base.resolve():
        _tmp = _base.parent / (_base.stem + '__c1_gate' + cpp.suffix)
        shutil.copyfile(cpp, _tmp)
        _src = _tmp
        _base_rel = _base.relative_to(ROOT).as_posix()
        _tmp_rel = _src.relative_to(ROOT).as_posix()
        for _n, _t in vars(bld).items():
            if isinstance(_t, dict) and _base_rel in _t:
                _t[_tmp_rel] = _t[_base_rel]
    obj = bld.compile_c(_src, skip_asm=False) if _src.suffix == '.c' else bld.compile_cpp(_src)
