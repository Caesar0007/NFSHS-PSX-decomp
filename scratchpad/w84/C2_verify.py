...
ROOT = Path(__file__).resolve().parents[2]   # W84-C2: script lives in scratchpad/w84/
...
if os.environ.get('NFS4_SOURCE_ONLY') == '1':
    for _name, _table in vars(bld).items():
        ...
        if _name == 'PER_FN_CC1_VER_SPLICE':
            ...
            continue
        _table.clear()
# --- W84-C2 LANE ALIASING (pattern: regiondiff/tools/verify_region.py ~120-130) ---
# Gate a scratchpad CANDIDATE copy of a USER-LIVE TU with the base TU's real lanes.
# The candidate is copied NEXT TO the base TU (sibling includes + directory-prefix
# class rules resolve exactly as the base) and every exact-rel-keyed lane table
# entry is aliased onto the temp path.  The base TU is never modified.
import shutil as _shutil
C2_BASE_REL = os.environ.get('C2_LANE_AS', 'recon/syslib/psx/libmcrd/LIBMCRD.c')
_c2_tmp = None
if cpp.resolve() != (ROOT / C2_BASE_REL).resolve():
    _c2_base = ROOT / C2_BASE_REL
    if not _c2_base.exists():
        sys.exit('C2 lane-as target not found: ' + C2_BASE_REL)
    _c2_tmp = _c2_base.parent / (_c2_base.stem + '__c2_gate' + cpp.suffix)
    _shutil.copyfile(cpp, _c2_tmp)
    _c2_base_rel = _c2_base.relative_to(ROOT).as_posix()
    _c2_tmp_rel  = _c2_tmp.relative_to(ROOT).as_posix()
    for _n, _t in vars(bld).items():
        if isinstance(_t, dict) and _c2_base_rel in _t:
            _t[_c2_tmp_rel] = _t[_c2_base_rel]
    cpp = _c2_tmp
# --- end lane aliasing ---
bld.OUT = bld.BUILD
...
_symtab_c2 = subprocess.run([OBJD, '-t', str(obj)], capture_output=True, text=True).stdout
if _c2_tmp is not None and _c2_tmp.exists():
    _c2_tmp.unlink()
...
if os.environ.get('C2_DUMP'):
    for fn in funcs:
        o=ours(fn); e=oracle(fn) or []
        n=max(len(o),len(e))
        for i in range(n):
            a=o[i] if i<len(o) else ''
            b=e[i] if i<len(e) else ''
            mark='   ' if a==b else '  *'
            print('%4d %-40s %s%s'%(i,a,mark,b))
    sys.exit(0)
