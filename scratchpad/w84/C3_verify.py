...
# W84-C3: COPY of tools/verify_asm.py living in scratchpad/w84/, so the repo root
# is two levels further up than the original's parent.parent.
ROOT = Path(__file__).resolve().parent.parent.parent
# The TU this copy stands in for (its real lane wiring lives under this key).
LANE_AS = "recon/syslib/psx/libmcrd/LIBMCRD.c"
...
# W84-C3: build.py does a bare `import fix_symsizes`, which only resolves when
# verify_asm.py IS the script in tools/ (30F-1).  A copy must add tools/ itself.
...
if os.environ.get('NFS4_SOURCE_ONLY') == '1':
    for _name, _table in vars(bld).items():
        ...
        _table.clear()
bld.OUT = bld.BUILD
# W84-C3 LANE ALIASING (catalog 30F-1): a copy-mode sibling inherits NO wiring, so
# every module-level dict keyed by the real TU's rel path gets the SAME value under
# this copy's rel path.  Without this the gate is vacuous (reads the default lane).
_cpp_rel = cpp.resolve().relative_to(ROOT).as_posix()
_aliased = []
_skip = set(filter(None, os.environ.get('C3_SKIP_TABLES', '').split(',')))
for _n, _t in vars(bld).items():
    if isinstance(_t, dict) and LANE_AS in _t:
        if _n in _skip:
            continue
        _v = _t[LANE_AS]
        _t[_cpp_rel] = dict(_v) if isinstance(_v, dict) else _v
        _aliased.append(_n)
# C3_TU_EXTRA="key=val,key2" -> merged into the ALIASED PER_TU_FLAGS copy only
# (the real TU's entry is never mutated: the value above is deep-copied).
if os.environ.get('C3_TU_EXTRA'):
    _tu = getattr(bld, 'PER_TU_FLAGS', {}).setdefault(_cpp_rel, {})
    for _kv in os.environ['C3_TU_EXTRA'].split(','):
        if not _kv:
            continue
        _k, _, _v2 = _kv.partition('=')
        _tu[_k] = _v2 if _v2 else True
# C3_ALT_MAP="name=C:/path/CC1PSX.EXE" -> register a VENDOR cc1 binary as a
# cc1_alt rung so NFS4_FORCE_CC1_ALT=name routes it through the direct-GNU-as
# (272-style) recipe.  Probe-only; retail vendor binaries exclusively.
if os.environ.get('C3_ALT_MAP'):
    _n2, _, _p2 = os.environ['C3_ALT_MAP'].partition('=')
    bld.CC1_SPECIAL_RUNGS[_n2] = [Path(_p2)]
if os.environ.get('C3_VERBOSE'):
    sys.stderr.write('lane-aliased %s -> %s via %s\n' % (LANE_AS, _cpp_rel, ','.join(_aliased)))
