...
/* REGIONAL (NFS4-R-USA): audioclc.obj gains one 4-byte small-data global
 * immediately BEFORE AudioClc_gCameraVelocity -- proven by the gp offsets:
 * base reads gCameraVelocity at %gp_rel = +0xBC, the regional slice reads it
 * at +0xC0 while the new store lands at +0xBC.  Region VA = 0x8013D8F8
 * (regional gCameraVelocity = 0x8013D8FC, address-taken in SoundCars).
 * [INFERRED] role: the single-screen "closest cars" master level returned by
 * the new region-only helper at 0x8007593C (0 in split-screen). Identity TBD. */
int          RDAT_8013D8F8;               /* region-layout @0x8013d8f8 (sbss) */
...
/* ---- NFS4-R-USA regional delta: audiocmn.obj-owned stereo flag.
 *   Retail gates both azimuth calculators on it (mono build -> azimuth 0).
 *   Identified via the regional MPause_MusicLogic slice (same address written
 *   1/0/1 by the three GameSetupWords[59] cases = base gStereoMode). */
extern int gStereoMode;   /* audiocmn.obj; region layout @0x8013d968 */

/* REGIONAL (NFS4-R-USA): region-only helper inserted between
 * AudioClc_SilenceOpponentHorn and AudioClc_SoundPlayersCar__Fi (61 insns).
 * Keeps the mechanical func_<region_va> name (no ground-truth name exists). */
extern "C" int AudioClc_SoundCloseCar__Fii(void);
...
extern "C" void AudioClc_SoundPlayersCar__Fi(int playerIndex,int closestIndex);
...
int AudioClc_CalcAzimuth(DRender_tCalcView *view,coorddef *object)
{
  ...
  /* REGIONAL (NFS4-R-USA): mono builds return a centred azimuth. */
  if (gStereoMode != 0) {
...
/* ---- AudioClc_SoundCloseCar__Fii  [REGION-ONLY @0x8007593C, NFS4-R-USA] ------------------
 * Region-only helper the retail build inserts between SilenceOpponentHorn and
 * SoundCloseCar; its result is stored in the new small-data global
 * RDAT_8013D8F8 by AudioClc_SoundCars on the single-screen path.
 * [INFERRED] role: an aggregate "how close is the pack" level in 0..128.
 * Globals identified STRUCTURALLY (never by regional address): the walked
 * table has stride 56 == sizeof(AudioClc_tSource) and its base sits 0xE0
 * below the view passed to AudioClc_CalcDistance -- the exact
 * gClosest[4] / gRenderView pair the base SoundCloseCar uses. */
extern "C" int AudioClc_SoundCloseCar__Fii(void)
{
  AudioClc_tSource *source;
  int i;
  ...
  total = 0xc00000;
  source = AudioClc_gClosest;
  for (i = 0; i < 4; i++) {
    Car_tObj *car = source->car;
    if (car != (Car_tObj *)0x0) {
      int dst = AudioClc_CalcDistance(&AudioClc_gRenderView,&(car->N).position);
      if (dst <= 0x7fffff) {
        total = (total - 0x800000) + dst;
      }
    }
    source++;
  }
  /* MIN/MAX macro pair, expanded as retail did: the MAX arm re-evaluates the
     MIN, which is why the `total / 0x10000` clamp appears twice. */
  return ((total / 0x10000 > 128 ? 128 : total / 0x10000) < 0)
             ? 0
             : (total / 0x10000 > 128 ? 128 : total / 0x10000);
}

/* ---- AudioClc_SoundPlayersCar__Fi__Fii  [@0x80075028] ---- */
extern "C" void AudioClc_SoundPlayersCar__Fi(int playerIndex,int closestIndex)
{
  AudioClc_tSource *source;
  Car_tObj *car;
  ...
  dir =
      (((fixeddiv(AudioClc_CalcCarDirection(&AudioClc_gRenderView,car),dst) <
        ...
        ((fixeddiv(AudioClc_CalcCarDirection(&AudioClc_gRenderView,car),dst) <
         ...
         0x10000) :
        -0x10000;
  /* REGIONAL (NFS4-R-USA): mono builds centre the close-car azimuth. */
  if (gStereoMode == 0) {
    azimuth = 0;
  }
...
extern "C" void func_80075F28(int playerIndex)
{
  ...
  dop = AudioClc_CalcDopplerShiftRatio(&(car->N).position,&(car->N).linearVel);
  /* REGIONAL (NFS4-R-USA): mono builds centre the player-car azimuth; gcc
     cross-jump-merges this `azimuth = 0` with the camera-mode-0 one below. */
  if (gStereoMode == 0) {
    azimuth = 0;
  }
  else if (GameSetup_gData.commMode == 1) {
...
void AudioClc_SoundCars(void)
{
  ...
  if (GameSetup_gData.commMode == 1) {
    ...
    RDAT_8013D8F8 = 0;   /* REGIONAL: split screen -> no close-car master level */
    func_80075F28(0);
    if (0 < gMasterAmbientLevel) {
      AudioClc_SoundPlayersCar__Fi(0, 0);
      AudioClc_SoundPlayersCar__Fi(0, 1);
    }
    ...
    func_80075F28(1);
    if (0 < gMasterAmbientLevel) {
      AudioClc_SoundPlayersCar__Fi(1, 2);
      AudioClc_SoundPlayersCar__Fi(1, 3);
    }
    ...
    RDAT_8013D8F8 = AudioClc_SoundCloseCar__Fii();   /* REGIONAL */
    func_80075F28(0);
    if (0 < gMasterAmbientLevel) {
      AudioClc_SoundPlayersCar__Fi(0, 0);
      AudioClc_SoundPlayersCar__Fi(0, 1);
      AudioClc_SoundPlayersCar__Fi(0, 2);
      AudioClc_SoundPlayersCar__Fi(0, 3);
    }
    AudioClc_gPlayer[0].lastview = AudioClc_gRenderView.translation;
  }
}
