...
/* REGIONAL (NFS4-R-UK-ES-IT): the base (USA) build stubs this out to "if the saved
   language is the -1 sentinel, force English".  The multi-language PAL SKU ships the
   REAL language-picker screen instead: it loads lang.psh, locates 9 shapes and runs a
   VSync'd pick loop over 3 languages until Start/Select is pressed.
   Callee identification (content-skeleton match of the regional slice's jal targets
   against NFS4-B-USA.EXE, NOT a base-symbol lookup of a regional VA):
     region 800E5094 == base 800E44BC sprintf          region 800E77DC == base 800E6C04 systemtask
     region 800F299C == base 800F1C3C loadshapeadr     region 800EBAB8 == base 800EB170 locateshapez
     region 800F146C == base 800F070C settrans         region 800F1498 == base 800F0738 movfxya
     region 800F2CA4 == base 800F231C VSync            region 800E6118 == base 800E5540 purgememadr
     region 80023D50 == base 80023C88 FEInput_GetKeyFromPlayer
     region 800A5130 == base 800A45B8 NFS4_LoadingIcon
   The two loaded globals in the sprintf call are the SAME pair DoTitleScreen above uses
   (STR_FRMT[0] format + Paths_Paths[0x20] art path); the shape names and language codes
   were read out of the regional image's rodata at the sprintf/table VAs:
     names "back","yENG","yITA","yESP","bENG","bITA","bESP","lite","dark"
     language codes { 0, 4, 3 } = ENG, ITA, ESP (the ESP code 3 selects the y=184 baseline) */
...

{
  ...
  if ((u_char)frontEnd.language == 0xff) {   /* sentinel: -1 == 0xFF as unsigned byte (this build's char is unsigned -> lbu;bne 0xFF) */
    /* decl ORDER == frame order: shapeNames@sp+16, shapes@sp+56, languages@sp+96,
       fullName@sp+104 (saved regs from sp+152; frame 184) -- these displacements are
       plain immediates, so the gate compares them exactly. */
    char *shapeNames[9] = {
      "back", "yENG", "yITA", "yESP", "bENG", "bITA", "bESP", "lite", "dark"
    };
    shapetbl *shapes[9];
    char languages[3] = { 0, 4, 3 };
    char fullName[48];
    char *shapeFile;
    int selected;
    int flash;
    ...
    int x;
    int shapeOffset;
    /* MATCH (LOAD-BEARING, do NOT "simplify" back to STR_FRMT[0]): the format
       pointer must be read through a pointer LOCAL, i.e. as an INDIRECT_REF of
       a non-aggregate VAR_DECL, so the MEM comes out WITHOUT MEM_IN_STRUCT_P.
       gcc-2.8 sched.c:849 (true_dependence) only lets a load cross a
       varying-address struct store when the LOAD is a non-struct fixed
       address; `STR_FRMT[0]` is an ARRAY_REF (/s set) and therefore stays
       CHAINED behind the 36-byte shapeNames[] block copy -- which pins the
       copy's 4-byte leftover before the `lw a1`, lengthens the copy's
       dest/src live ranges, and hands global-alloc a different caller-saved
       trio (ours dest=v1/src=v0/limit=a0 vs retail dest=a2/src=v1/limit=v0).
       That single alias edge WAS the whole 34-diff block-copy residual.
       (`*(char **)STR_FRMT` does NOT work: expand_expr's INDIRECT_REF case
       sets MEM_IN_STRUCT_P for an ADDR_EXPR of an aggregate.)
       Paths_Paths[0x20] must STAY an ARRAY_REF -- retail keeps ITS load below
       the leftover copy, i.e. that dependence is real in the oracle too. */
    char **fmt = STR_FRMT;

    selected = 0;
    sprintf(fullName,*fmt,Paths_Paths[0x20],"lang.psh");
    shapeFile = (char *)loadshapeadr(fullName,(void *)0x0);
    systemtask(0);
    i = 0;
    do {
      shapes[i] = (shapetbl *)locateshapez(shapeFile,(void *)shapeNames[i]);
      i = i + 1;
    } while (i < 9);
    flash = 0;
    Quick_DD(1,1,1);
    settrans(0);
    movfxya(shapes[0],0,0);
    key = 0;
    while ((key != 2) && (key != 0x2000)) {
      ...
      settrans(1);
      /* shapes[7] = "lite" (selected marker), shapes[8] = "dark" (unselected).
         MATCH: statement ORDER is the sealed FR-DE row's -- x, then VSync(0),
         then i, then shapeOffset.  With `i = 0` written first the VSync
         argument's zero is materialized by `i` (`addu s1,zero,zero;
         addu a0,s1,zero`); the retail pair is the other direction
         (`addu a0,zero,zero; addu s1,a0,zero`), i.e. cse gives the constant
         to the FIRST-assigned pseudo and the VSync argument must be it. */
      x = 0xf6 - shapes[7]->width - (shapes[8]->width >> 1);
      VSync(0);
      i = 0;
      shapeOffset = 4;
      /* MATCH: a real `for` with both arms of the selected/unselected offset
         ASSIGNING (§5.0c #3) -- the default+override form
         (`sel = shapeOffset; if (selected != i) sel = (i+4)*4;`) hoists the
         default into a live scratch BEFORE centerOffset (`addu a3,s2,zero`)
         and pushes the centerOffset `subu` into the beq delay slot; retail
         computes centerOffset first and puts the default IN the delay slot. */
      for (; i < 3; i++) {
        int markerOffset;
        int centerOffset;
        ...
        movfxya(*(shapetbl **)((char *)shapes + markerOffset),x,0xb4);
        centerOffset = (shapes[7]->width >> 1) -
                       ((*(shapetbl **)((char *)shapes + shapeOffset))->width >> 1);
        if (selected == i) {
          selectedShapeOffset = shapeOffset;
        }
        else {
          selectedShapeOffset = (i + 4) * 4;
        }
        shapePtr = (shapetbl **)((char *)shapes + selectedShapeOffset);
