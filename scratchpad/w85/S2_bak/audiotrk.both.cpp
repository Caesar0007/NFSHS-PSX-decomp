...
AudioTrk_find_channel:
      ...
      for (i = 0; (c == (AudioTrk_tAmbientChannel *)0x0) && (i < 0x10); i++) {
        if (AudioTrk_g->chan[i].se == (AudioElem *)0x0) {
          c = AudioTrk_g->chan + i;
          /* W85-S2: an identity fence `__asm__("" : "=r"(c) : "0"(c))` used to sit
             here, claimed to stop cse substituting `c` for the freshly computed
             `&chan[i]`.  MEASURED EXACTLY INERT (PASS 358/358 with it, without it,
             and in combination with the se->type fence below) -- DELETED. */
...
AudioTrk_channel_found:
      ...
      if (c->se != (AudioElem *)0x0) {
        ...
        if (dst < (int)se->range << 0x10) {
          if (se->type == '\x03') {
            /* W85-S2: an `'m'`-fence `__asm__("" : "+m"(se->type))` used to sit here,
               claimed to stop GCC reusing the known value 3 across the merge.
               MEASURED EXACTLY INERT (PASS 358/358 with and without) -- DELETED. */
            azimuth += trkazi;
          }
          else if (se->type != '\x02') {
