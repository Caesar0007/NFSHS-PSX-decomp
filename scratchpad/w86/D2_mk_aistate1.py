...
S_OLD = '  __asm__("" : : "r"(search));\n'
S_NEW = ('  /* W86-D2: read-only ref fence -> pure-C ABSORPTION IDENTITY `X | (X & 3) == X`.\n'
...
I_OLD = ('      /* W54-A15 REF-STEP #2 (reqdelta): +1 ref on inRange (3->4 floor_log2 step) rotates the\n'
         '         desiredSpeed/limit/inRange trio onto retail\'s $a3/$a2/$v0; 0 insns. */\n'
         '      __asm__("" : : "r"(inRange));\n')
I_NEW = ('      /* W54-A15 REF-STEP #2 (reqdelta): +1 ref on inRange (3->4 floor_log2 step) rotates the\n'
         '         desiredSpeed/limit/inRange trio onto retail\'s $a3/$a2/$v0; 0 insns.\n'
...
pathlib.Path('scratchpad/w86/D2_as1.txt').write_text(
    repr([('aistate 2 clears', [(S_OLD, S_NEW), (I_OLD, I_NEW)])]))
print('ok')
