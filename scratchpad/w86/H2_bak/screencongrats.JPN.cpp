...
/* REGIONAL DELTA callee.  The base object never calls it, so no header in this
   TU's include chain declares it and cc1plus falls back to an implicit typing
   that passes the RECT BY VALUE (packed into $a0/$a1 + the opacity in $a2) --
   the regional oracle passes ONE address (`addiu $a0,$sp,24; li $a1,1`).  The
   signature is the base tree's own definition in recon/frontend/psx/drawshp.cpp.
   Declared locally because that shared header is a base-tree file this regional
   agent must not edit. */
...

{
  ...
  if (this->congratsMessage == kScreenCongrats_Congrats) {
    ...
    /* REGIONAL DELTA (JPN): both message renders switch to textState_Hilighted,
       the first render's height advance carries a -14 bias applied to the OLD
       r.y (oracle `lhu r.y; addiu -14; addu <call result>`), and the second
       render narrows the box to 200 first. */
    if (firstmessage != 0) {
    ...
    if (tInfo.fAwardCarGarageFull != 0) {
      /* REGIONAL DELTA (JPN): the award box is repositioned (y 200 -> 100,
         w 0xf0 -> 220), the WordWrap call now renders the FORMATTED `buffer`
         instead of re-fetching the raw TextSys_Word(0x40) format string, and
         retail then shrink-wraps `r` around the rendered text and paints a
         translucent plate behind it. */
      r.x = 0x104;
      r.y = 100;
      r.w = 220;
      ...
      FETextRender_WordWrapTextFade
                ((int)this->fScreenFadeVal,buffer,r,textState_Selected,
                 textType_PostGame);
      r.h = (short)(FETextRender_WordWrapHeight(r.w,buffer) + 6);
      r.y = 0x61;
      r.x = r.x - 5;
      r.w = r.w + 10;
      DrawShape_NFS4TransRectangle(r,1);
    }
  }
  else {
    int yyy;
