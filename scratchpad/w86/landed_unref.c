        ...
        if (reverse != 0) {
            ...
            for (;;) {
                ...
                if ((op & 0x80) == 0) {                   /* 2-byte command */
                     ...
                     * Measured (count-EXACT 158/158 in EVERY row, oracle 158): no dial 122
                     * diffs | op-reads +1 122 | +2..+3 78 | +4 with src 0 (PASS).  Do NOT
                     * "simplify" either expression away -- see scratchpad/w86/D1_receipt.md. */
                    ...
                    lo    = ((op >> 16) & 0xff) + 1;
                    count = hi + lo;
                    len   = (int)((((op & 0x3f) & op) & op) & op) + 4;   /* == op & 0x3f; W86-D1 ref dial */
