"""S2 scope audit: SYM block-nesting depth of each local vs the C decl's brace depth.

usage: s2_scope.py <recon.cpp>
"""
import sys, re
...
for blk in blocks:
    ...
    for l in blk:
        ...
        if mm and mm.group(1) not in ('REGPARM', 'ARG') and mm.group(2) not in ('this', '__in_chrg'):
            syms.setdefault(mm.group(2), []).append(d)
    # C decl depths
    cdep = {}
    for mm in re.finditer(r'^[ \t]+([A-Za-z_][\w \t*]*?)\b([A-Za-z_]\w*)\s*(\[[^\]]*\])?\s*(=[^;]*)?;\s*$',
                          body, re.M):
        nm2 = mm.group(2)
        if mm.group(1).strip() in ('return', 'delete', 'goto', 'else', 'case', 'new', 'break'): continue
        dep = body[:mm.start()].count('{') - body[:mm.start()].count('}')
        cdep.setdefault(nm2, []).append(dep)
    issues = []
    for nm2, ds in syms.items():
        if nm2 not in cdep: continue
        s_min = min(ds); c_min = min(cdep[nm2])
        s_fnscope = (s_min <= 1)
        c_fnscope = (c_min <= 1)
        if s_fnscope != c_fnscope:
            issues.append(f"{nm2}: SYM block-depth {sorted(set(ds))} vs C brace-depth {sorted(set(cdep[nm2]))}")
    if issues:
        bad += len(issues)
        print(f"### {fn} -> {dm}")
        for x in issues: print("    " + x)
        print()
print(f"# scope mismatches: {bad}")
