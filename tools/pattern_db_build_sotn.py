# Phase-3: mine Castlevania: SOTN matched C (github.com/Xeeynamo/sotn-decomp, AGPL-3.0) into the pattern DB.
# SOTN's own toolchain is gcc 2.6.3, but we compile its MATCHED C with the SH cc1psx 2.8.1 (a C compiler
# close to our gcc 2.8.0) so the indexed SHAPES land in OUR query space; the matched C is the authentic
# idiom either way. The project is INCOMPLETE -> -DSKIP_ASM neutralizes INCLUDE_ASM (include/include_asm.h)
# so ONLY MATCHED functions compile (the unmatched stubs vanish; no ROM / no extracted .s needed).
# PSX overlays only (exclude saturn=SH2 ISA, *_psp=PSP/newer-gcc, pc=PC-port reimpl).
# Usage: python tools/pattern_db_build_sotn.py [src/overlay ...]   (default: all PSX overlays)
# MERGES into tools/pattern_db.json (must already contain the NFS4 + SH records). Idempotent (replaces the SOTN portion).
import subprocess, re, json, sys
from pathlib import Path
NFS4=Path(__file__).resolve().parent.parent
SOTN=Path(r"C:/Temp/sotn-decomp")
...
PSX_OVERLAYS=['boss','dra','main','maria','ric','sd','servant','st','weapon']
INC=['-I'+str(SOTN/'include'),'-I'+str(SOTN/'include/psxsdk'),'-I'+str(SOTN/'src')]
def norm(t):
    ...
    t=re.sub(r'\((\d+) ?>> ?(\d+)\)',lambda m:str(int(m.group(1))>>int(m.group(2))),t)  # eval (N>>M) — parity w/ verify_asm
...
def compile_sotn(c):
    i=SCR/'sotnx.i'; s=SCR/'sotnx.s'; o=SCR/'sotnx.o'
    r=subprocess.run([GCC,'-E','-nostdinc','-undef',*INC,'-D__GNUC__=2','-Dpsx','-D__psx__','-D__mips__',
                      '-DSKIP_ASM','-DVERSION_US',str(c),'-o',str(i)],capture_output=True,text=True)
    ...
    return o
def src_funcs(c):   # local per-file source extraction (SOTN has NO #include "*.c" -> a fn's body lives in its own file)
...
records=[r for r in db['records'] if r.get('game')!='sotn']; base=len(records)   # idempotent: replace SOTN portion
subdirs=sys.argv[1:] or ['src/'+d for d in PSX_OVERLAYS]
cfiles=[]
for sd in subdirs: cfiles+=sorted((SOTN/sd).rglob('*.c'))
cfail=0; seen=set()
for ci,c in enumerate(cfiles):
    if ci%50==0: print(f"[{ci+1}/{len(cfiles)}] {c.name} (sotn recs:{len(records)-base} fail:{cfail})",flush=True)
    o=compile_sotn(c)
    if o is None: cfail+=1; continue
    funcs=obj_funcs(o); srcs=src_funcs(c)
    for fn,ins in funcs.items():
        if not ins or len(ins)<3 or fn in seen: continue   # dedup by name (st/ repeats entity boilerplate across stages)
        src=srcs.get(fn)
        if src is None: continue   # no local matched C (inlined-from-header / asm-only) -> skip
        seen.add(fn)
        records.append({'func':fn,'file':'SOTN/'+str(c.relative_to(SOTN)).replace('\\','/'),'n':len(ins),
                        'asm':ins,'src':src,'compiler':'gcc2.8.1','game':'sotn'})
...
sotn=len(records)-base
print(f"SOTN added: {sotn} records (compile-fail files: {cfail}/{len(cfiles)}) | total records: {len(records)} | shapes: {len(index)} | SOTN src-attached: {sum(1 for r in records[base:] if r['src'])}/{sotn}")
