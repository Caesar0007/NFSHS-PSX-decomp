...
for n in (5,4,3,2):
    for i in range(len(qn)-n+1):
        ...
        for rid in index.get(key,[]):
            hits[rid]+=n*n            # longer windows weigh much more
            shapes.setdefault(rid,set()).add(key)
print(f"query: {len(qn)} insns -> {qn}\n")
if not hits: sys.exit("no matching sealed shapes found.")
for rid,score in hits.most_common(6):
    r=records[rid]
    best=sorted(shapes[rid], key=lambda k:-k.count('|'))[:2]
    print(f"=== {r['func']}  [{r['file']}]  score={score}  ({r['n']} insns) ===")
