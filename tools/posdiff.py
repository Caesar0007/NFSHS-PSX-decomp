...
import collections, os, difflib
o = ours(fn)
e = oracle(fn)
REG = r'\b(v[01]|a[0-3]|t[0-9]|s[0-7]|fp)\b'      # allocatable only
def alpha(seq):
    m={}; out=[]
    for ins in seq:
        def sub(mm):
            r=mm.group(1)
            if r not in m: m[r]='R%d'%len(m)
            return m[r]
        out.append(re.sub(REG,sub,ins))
    return out,m
ao,mo = alpha(o); ae,me = alpha(e)
print("counts ours=%d oracle=%d" % (len(o), len(e)))
print("ours  first-use order: %s" % ' '.join(k for k,_ in sorted(mo.items(),key=lambda kv:int(kv[1][1:]))))
print("oracle first-use order: %s" % ' '.join(k for k,_ in sorted(me.items(),key=lambda kv:int(kv[1][1:]))))
sm=difflib.SequenceMatcher(None,ao,ae,autojunk=False)
same=sum(i2-i1 for t,i1,i2,j1,j2 in sm.get_opcodes() if t=='equal')
print("ALPHA-RENAMED LCS match: %d / %d  (structural residual %d)" % (same,len(ae),len(ae)-same))
mode=os.environ.get('MODE','sum')
if mode=='diff':
    for tag,i1,i2,j1,j2 in sm.get_opcodes():
        ...
        for k in range(max(i2-i1,j2-j1)):
            ...
            print("%-6s OURS %-34s ORACLE %s" % (tag,ol,el))
