...
| Per-object type records | `dedup_types.py` (one definition per duplicated game type: identical copies move verbatim to `recon/shared/<Type>.h`, each copy becomes a padded include; SYM text unchanged by construction; `--canon` also merges copies that differ only in spelling per `type_canon.py`, keeping an alias-free spelling -- gate with a full debug-SYM compare), `member_cmp.py` (retail vs ours member records per type and source file; `--summary` classifies shared types), `header_groups.py` (candidate original headers from retail type co-occurrence), `tagset_cmp.py` (per source file: retail vs ours STRTAG/UNTAG/ENTAG/TPDEF sets and per-kind order; `--core` lists the retail-universal core that `recon/nfs4_types.h` holds; `--file X` details one file) | `sym-match.md` |
| SLD / scope probes and reference upkeep | `sldprobe.py` (real TU: per-instruction line tags and `.begin/.bend` against retail, aligned to the real object), `gprobe.py` (standalone `-g` probe), `var_fn.py` (splice a candidate function, verify bytes + probe), `ref_refresh.py` (re-adopt a stale `symloop` byte reference on the honest-link proof), `simplemem_inline.py` (carrier -> file-scope inline), `implicit_sweep.py` / `implicit_fix.py` (undeclared callees create debug scopes: list them, add prototypes), `equ_survey.py`, `slink_census.py` | `sym-match.md`, `BUILDING.md` |
