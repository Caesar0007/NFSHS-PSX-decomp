...
| SLD / scope probes and reference upkeep | `sldprobe.py` (real TU: per-instruction line tags and `.begin/.bend` against retail, aligned to the real object), `gprobe.py` (standalone `-g` probe), `var_fn.py` (splice a candidate function, verify bytes + probe), `ref_refresh.py` (re-adopt a stale `symloop` byte reference on the honest-link proof), `simplemem_inline.py` (carrier -> file-scope inline), `equ_survey.py`, `slink_census.py` | `sym-match.md`, `BUILDING.md` |
