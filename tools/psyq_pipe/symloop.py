...
r = subprocess.run([PY, str(ROOT / 'tools/psyq_pipe/gdebug_compile.py'), *frs], capture_output=True, text=True, cwd=ROOT)
for ln in r.stdout.splitlines():
    if 'DIFF' in ln or 'FAILED' in ln:
        print('  [-g]', ln[:150])
env = dict(os.environ, NFS4_LANE_G='1', NFS4_LANE_OUT='build/psyq_g', NFS4_LANE_ONLY=','.join(frs))
r = subprocess.run([PY, str(ROOT / 'tools/psyq_pipe/psylink_lane.py')], capture_output=True, text=True, cwd=ROOT, env=env)
for ln in r.stdout.splitlines():
    if ln.startswith('ASPSX ok') or ln.startswith('PSYLINK:'):
        print(' ', ln[:120])
subprocess.run([PY, str(ROOT / 'tools/psyq_pipe/symtree_cmp.py'), str(ROOT / 'build/psyq_g/nfs4_sym.txt')], capture_output=True, text=True, cwd=ROOT)
