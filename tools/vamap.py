#!/usr/bin/env python3
"""vamap -- GLOBAL name -> retail VA map (W64-A18; PROMOTED W65-A7).
...
Used by ownmap's reloc-aware E5 leg: an R_MIPS_32 word in our data section
holds only its ADDEND until link, so the raw bytes can never equal retail.
Resolving it needs the target symbol's retail VA.

🔴 PROMOTION NOTE (catalog 16F -- CHECK ROOT RESOLUTION ON EVERY PROMOTION):
the scratchpad copies use `parents[2]` (scratchpad/<agent>/x.py).  In tools/
the repo root is `parents[1]`.  Getting this wrong makes every lookup MISS
...
def build(verbose=False):
    ...
    assert n1, "symbol_addrs.txt yielded no names -- refusing a vacuous map"
    return m, src
