"""Variant probe harness: apply N source variants to a TU, gate each, restore.

Usage:  python tools/varprobe.py <variants.py>

The variants module must define:
    TU   = 'recon/game/psx/hud.cpp'
    FNS  = 'MangledA,MangledB'
    VARIANTS = [(name, old_text, new_text), ...]      # single anchor
             or [(name, [(old, new), (old, new), ...]), ...]  # multi anchor

Every variant is applied to the REAL path (PER_TU flags are path-keyed -- a
renamed copy silently loses them), gated, and the file restored in a finally
block.  Anchors are written with '
' and re-punctuated to the file's own line
ending, so the same variants file works on LF and CRLF TUs.
"""
